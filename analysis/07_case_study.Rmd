...
210/389 # 0.539845at of cells are not threatened
test <- iucn_plot_df %>% 
...
mean(test$prop_at_risk_impacted, na.rm = TRUE) # 0.07497449 forage fish - of the forage fish cells, only ~7.5% of species in the cells are at-risk

test <- iucn_plot_df %>% 
  filter(max_iucn_rank == 2) %>%
  left_join(raw_mat_summary) %>%
  group_by(dom_raw_material) %>%
  summarise(prop_mean = mean(prop_at_risk_impacted,na.rm = TRUE)) %>%
  ungroup()


# # A tibble: 5 × 2
#   dom_raw_material prop_mean
#   <chr>                <dbl>
# 1 Cropsnes            0.104 
# 2 Forage fish         0.0750
# 3 Pulses              0.529 
# 4 Rapeseed            0.250 
# 5 Wheat               0.169
...
# # A tibble: 5 × 3
#   dom_raw_material `n()`   prop
#   <chr>            <int>  <dbl>
# 1 Cropsnes            33 0.184 
# 2 Forage fish         76 0.425 
# 3 Pulses               2 0.0112
# 4 Rapeseed            61 0.341 
# 5 Wheat                7 0.0391
