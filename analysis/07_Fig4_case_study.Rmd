...
nrow(test)/(nrow(iucn_plot_df)) # 0.5578035 of cells are not threatened

test <- iucn_plot_df %>% 
  filter(max_iucn_rank == 2, prop_at_risk_impacted == 1)
nrow(test)/nrow(iucn_plot_df)
...
ggsave(here("analysis/SI_plots/SI_hotspot_CAN_quantile.png"), dpi = 300, bg = "white", height = 16, width = 16, units = "cm")
...
ggsave(here("analysis/SI_plots/SI_hotspot_CAN_iucn_new.png"), dpi = 300, bg = "white",  height = 16, width = 16, units = "cm")
...
ggsave(here("analysis/SI_plots/SI_CAN_base_map.png"), plot = map_plot, dpi = 300, bg = "white", height = 16, width = 16, units = "cm")
...
test <- iucn_plot_df %>% 
  filter(max_iucn_rank == 1)
7/975 # 0.7% of cells are not threatened
test <- iucn_plot_df %>% 
  filter(max_iucn_rank == 2, prop_at_risk_impacted == 1)
344/975 # 35% of cells are at risk and all species in those cells are at risk 

summary_distinct <- distinct(summary, cell_id, avg_impact)

sum(summary_distinct$avg_impact) # 2.905138

chl_marine <- max_impact_iso3c_marine %>%
  filter(iso3c == "CAN", diet == "plant-dominant")
sum(chl_marine$impact_km2) # 117.7947

chl_terr <- max_impact_iso3c_terr %>%
  filter(iso3c == "CAN", diet == "plant-dominant")
sum(chl_terr$impact_km2) # 46936.8

max_impact <- max_impact_iso3c_terr %>%
  filter(iso3c == "CAN") %>% # lets check out Chile
  rbind(max_impact_iso3c_marine %>% filter(iso3c == "CAN")) %>%
  filter(x < -6000000) %>%
  filter(x > -7000000,
         y < 5500000)

sum(max_impact$impact_km2)/(sum(chl_terr$impact_km2) + sum(chl_marine$impact_km2)) # 16% - this hotspot represents 16% of all aquafeed impacts in Canada's land area and EEZ. 

975*100 # 97500 km2; Canada land and eez area is 9900000 + 5599077 km2 (wikipedia)
# 97500/15499077 # 0.6%
