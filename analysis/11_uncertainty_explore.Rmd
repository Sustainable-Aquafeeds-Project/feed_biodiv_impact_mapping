...
ggplot(data = loop_df_all, aes(x = mean_prop, y = sd_prop)) +
  geom_point() +
  geom_smooth(se = FALSE) +
  labs(caption = glue("{allocation_type}")) + 
  facet_grid(fcr_type~diet, scales = "free") 

ggsave(glue(here("analysis/uncertainty/mean_sd_scaling_{allocation_type}.png")))
...
for(allocation_type in allocation_types){
      ...
      imp_rast <- 1-rast(glue(file.path(biodiv_dir, "output/impact_maps_across_taxon_ingredient/{diet_type}/{fcr_type}/{allocation_type}_mean_prop.tif")))
...
png(file = glue(here("prep/03_prep_spp_habitats/output/plots/uncertainty/{diet_type}_{fcr_type}/high_impact.png")), width = 1200, height = 700)
plot(high_imp_sum, col = pal(3), type = "classes") 
dev.off()

png(file = glue(here("prep/03_prep_spp_habitats/output/plots/uncertainty/{diet_type}_{fcr_type}/low_impact.png")), width = 1200, height = 700)
...
png(file = glue(here("prep/03_prep_spp_habitats/output/plots/uncertainty/{diet_type}_{fcr_type}/disagree.png")), width = 1200, height = 700)
plot(r)
dev.off()
