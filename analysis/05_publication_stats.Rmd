...
unique(test$diet) # [1] "fish-dominant"
unique(test$taxon) #  [1] "Sponges"        "Arthropods"     "Finfish"        "Cephalopods"    "Cnidaria"       # "Molluscs"      
#  [7] "Reptiles"       "Echinoderms"    "Marine plants"  "Corals"         "Elasmobranchs"  "Marine mammals"
# [13] "Birds"          "Amphibians"  
unique(test$ingredient) # [1] "forage fish_fish meal" "forage fish_fish oil" 

# interesting.. only one without a 10% decrease in impacts from regular to efficient are fish-dominant - forage fish FMFO
# why is this happening only in fish-dominant? 
...
forage_fd <- fcr_comparison_df %>%
  filter(diet == "fish-dominant",
         ...
         percent_difference != 0) %>%
  filter(percent_difference < 0)
mean(forage_fd$percent_difference) #  [1] -17.18885.. so for some reason, forage fish FMFO under fish dominant is WORSE under the efficient scenario

unique(forage_fd$spp_type) # [1] "sponges"       "arthropods"    "fish"          "cephalopods"   "cnidaria"      "molluscs"      "Reptiles"     
#  [8] "echinoderms"   "Marine plant"  "corals"        "elasmobranchs" "Marine mammal" "Bird"          "Amphibians" 
# all species

## ok thats weird... lets check the global rasters:
...
## ok we see the same thing here... all 10% decrease for plant-dominant but fish-dominant is difference, specifically ocean cells (so forage fish)

## lets check out the forage fish pressure rasters? 

# take forage fish fish oil, pelagic catch

fd_eff_rast <- rast(file.path(biodiv_dir, "int/resampled_ingredient_rasts/fish-dominant/efficient/forage fish_fish meal_economic_benthic_catch_A.tif"))

fd_reg_rast <- rast(file.path(biodiv_dir, "int/resampled_ingredient_rasts/fish-dominant/regular/forage fish_fish meal_economic_benthic_catch_A.tif"))


diff_rast <- (fd_reg_rast - fd_eff_rast)/fd_reg_rast

plot(diff_rast)



pd_eff_rast <- rast(file.path(biodiv_dir, "int/resampled_ingredient_rasts/plant-dominant/efficient/forage fish_fish oil_economic_pelagic_catch_P.tif"))

pd_reg_rast <- rast(file.path(biodiv_dir, "int/resampled_ingredient_rasts/plant-dominant/regular/forage fish_fish oil_economic_pelagic_catch_P.tif"))


diff_rast <- (pd_reg_rast - pd_eff_rast)/pd_reg_rast
