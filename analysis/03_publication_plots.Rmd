...
plot1 <- ggplot(summary_df_spp %>% filter(prop_impact > 0), aes(x = prop_impact)) +
  ...
  # geom_point(data = summary_df_spp, aes(x = mean_prop, y = taxon, group = diet, fill = terr_marine),
  #            position = position_dodge(width = -0.9),
  ...
  scale_color_manual(values = diet_cols, breaks = names(diet_cols), guide = guide_legend(reverse = FALSE)) +
   # scale_fill_manual(values = c("Marine" = "lightblue", "Terrestrial" = "darkgreen")) +  # Customize fill colors for terr_marine
...
plot2 <- ggplot(summary_df_spp %>% filter(prop_impact > quant_95), aes(x = prop_impact)) +
  ...
  theme_ohara() +
      labs(x = "", y = "") +
  scale_x_continuous(expand = c(0,0), limits = c(quant_95, 0.018), breaks = c(quant_95, seq(0.003, 0.018, by = 0.006)), labels = c("0.000055", "0.003", "0.009", "0.015")) +
  scale_color_manual(values = diet_cols, breaks = names(diet_cols), guide = guide_legend(reverse = FALSE)) +
  theme(panel.background = element_blank(),
            axis.title.y = element_blank(),
            axis.text.x = element_text(size = 16, angle = 45, hjust = 1),
