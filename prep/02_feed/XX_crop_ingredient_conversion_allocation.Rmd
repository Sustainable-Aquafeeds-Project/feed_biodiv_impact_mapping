...
```{r}
## read in raw material extraction rates (yields) and gross energy data that me and Rich compiled 

#get yield data from main ingredients
cf_data <- read.csv("data/tidy_data/allocation/crop_ingredient_allocation_factors.csv") 
...
setdiff(unique(ingredient_production_location$source_ingredient), unique(cf_data$ingredient)) # none

setdiff(unique(cf_data$FAO_code), unique(ingredient_production_location$FAO_code)) # none

setdiff(unique(ingredient_production_location$FAO_code), unique(cf_data$FAO_code)) # none
...
ingredient_production_cf <- ingredient_production_location %>%
  ...
  ungroup() %>%
  left_join(cf_data, by = c("source_ingredient" = "ingredient", "FAO_code")) %>%
  dplyr::select(iso3c_producing, SPAM_super, diet, source_ingredient, raw_material, FAOSTAT_name, FAO_code, producing_ingredient_consumed_tonnes, cf = yield, coproduct_cf = yield_coproduct, energy_cf_product, energy_cf_coproduct, gross_energy_product)



## Ok so now it is gapfilled, we need to calculate the mass allocation factor for each ingredient
allocation_raw <- ingredient_production_cf %>%
  filter(!is.na(diet)) %>%
  group_by(source_ingredient, iso3c_producing, diet) |> 
  nest() |> 
...
sum(tmp$included_crop)/(sum(tmp$included_crop) + sum(tmp$excluded_crop)) # 93% included and does not need to be distributed for ms, ~90% for ge
...
tmp <- filter(feed_source) %>%
  ...
  slice_max(order_by = produced_ingredient_tonnes_ms, n = 15) %>%
  ungroup() %>%
  filter(produced_ingredient_tonnes_ge > 0)


ggplot(data = tmp %>%
         filter(diet == "fish-dominant"), aes(x = produced_ingredient_tonnes_ms, y = reorder_within(iso3c_producing, produced_ingredient_tonnes_ms, SPAM_super))) + 
  geom_bar(stat = "identity") + 
...
ggsave(here("prep/02_feed/plots/total_crop_feed_origins_fish_mass.png"), units = "in", height = 11, width = 11)
```
