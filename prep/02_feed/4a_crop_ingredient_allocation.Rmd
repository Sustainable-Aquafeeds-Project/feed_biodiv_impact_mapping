...
total_prop <- read.csv(here("prep/02_feed/output/proportion_feed_per_country_system_diet.csv")) %>%
  dplyr::select(SPAM_super, diet, ingredient = source_ingredient, iso3c_producing, total_prop_ms = prop_produced_for_system_ms, total_prop_ge = prop_produced_for_system_ge) %>%
...
mapspam_fao <- feed_cats %>%
  dplyr::distinct(FAO_code, map_spam_code)
...
fao_yields <- readRDS(here("data/tidy_data/production-data/crops_production_tidy.rds")) %>%
  ...
  left_join(mapspam_fao, by = c("item_code" = "FAO_code")) %>% # get mapspam codes
  filter(!is.na(map_spam_code))
...
tonnes_crop_raster <- rast(sprintf(file.path(mapspam_dir, "scaled_prod_maps_2021/crop_%s_A_scaled.tif"), crop_spam))

tonnes_crop_feed_raster <- tonnes_crop_raster*prop_feed_crop_raster

# plot(log(tonnes_crop_feed_raster+1))

writeRaster(tonnes_crop_feed_raster, filename= sprintf(here("prep/02_feed/output/%s/%s_%s_%s_P.tif"), diet_type, crop_spam, product_type, allocation), overwrite=TRUE)

# country yield estimates

feed_yield <- filter(fao_yields, map_spam_code == crop_spam) %>%
...
# plot(log(area_crop_feed_raster+1)) # 16197.75 hectares of corn produced for corn gluten feed.. reasonable?
...
writeRaster(area_crop_feed_raster, filename= sprintf(here("prep/02_feed/output/%s/%s_%s_%s_A.tif"), diet_type, crop_spam, product_type, allocation), overwrite=TRUE)
...
test_names <- str_replace_all(str_before_last(list.files(here("prep/02_feed/output/plant-dominant/"), pattern = "mass_P.tif"), ".tif"), "_", ":")

test <- rast(list.files(here("prep/02_feed/output/plant-dominant/"), full.names = TRUE, pattern = "mass_P.tif"))

names(test) <- test_names
plot(log(test+1))
test_df <- as.data.frame(global(test, "sum", na.rm = TRUE))
sum(test_df$sum) # 10881.41 km2 of cropland for salmon aquaculture feeds? 
# 2927363 tonnes of crops for salmon aquaculture feeds 
# 2804335 demanded.. so pretty close there. 
2927363/10881.41 # 269 tonnes/km2 
10881.41/2927363 # 0.003717137 km2/tonne 
(10881.41*1000000)/(2927363*1000) # 3.717137 m2 per kg.. EU chicken meat is 5.59 m2 per kg
