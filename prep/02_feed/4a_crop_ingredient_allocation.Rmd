...
nor_trade <- trade_data %>% 
  filter(iso3c_consuming == "NOR" | iso3c_producing == "NOR") %>%
  filter(GAEZ_category %in% c("Rapeseed", "Soybean"))


```


Total demand by animals per country in MAPSPAM crops
...
feed_cats <- read_xlsx(here("data/tidy_data/plant_ingredient_codes.xlsx")) %>% ## this table was created manually. It matches our ingredient names from Aas et al. 2022 to GAEZ raw material crop names. Must also update product_to_fao_spam.csv. Taken from our IMAS Sustainable Aquafeed Project - Mapping directory on OneDrive, specifically the "allocation_new" folder. 
