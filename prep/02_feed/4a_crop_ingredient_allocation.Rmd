...
feed_source <- ingredient_production_location %>%
  group_by(iso3c_producing, diet, animal_system, GAEZ_category, source_ingredient) %>%
...
ingredient_production <- ingredient_production_location %>%
  group_by(iso3c_producing, GAEZ_category, diet, source_ingredient, raw_material, FAOSTAT_name, FAO_code, map_spam_code) %>%
...
allocation_factors <- read_xlsx(here("data/tidy_data/allocation/crop_ingredient_allocation_factors.xlsx")) %>% 
  dplyr::select(item, ingredient, FAO_code, map_spam_code, gaez_code, ge_allocation_factor, mass_allocation_factor) %>%
  distinct()
...
production_exceed <- proportion %>%
  group_by(iso3c_producing, diet, GAEZ_category, source_ingredient) %>%
...
tonnes_lost_system <- tonnes_included_system %>%
  group_by(GAEZ_category, diet, source_ingredient) %>%
...
tonnes_extra_crop <- corrected_prop %>%
  dplyr::select(iso3c_producing, diet, SPAM_super, GAEZ_category, extra_crop_tonnes_ms, tonnes_producing_crop, extra_crop_tonnes_ge, source_ingredient) %>%
  unique() %>%
...
assigned_prop <- tonnes_included_system  %>%
  dplyr::select(iso3c_producing, diet, source_ingredient, SPAM_super, GAEZ_category, prop_produced_for_system_ms = included_prop_ms, prop_produced_for_system_ge = included_prop_ge)
excess_prop <- excess_prop %>%
  dplyr::select(iso3c_producing, diet, source_ingredient, SPAM_super, GAEZ_category, prop_extra_for_system_ms = prop_dispersed_ms, prop_extra_for_system_ge = prop_dispersed_ge)

total_prop <- left_join(assigned_prop, excess_prop, by=c("iso3c_producing", "diet", "GAEZ_category", "source_ingredient")) %>%
  rowwise() %>%
...
total_prop <- read.csv(here("prep/02_feed/output/proportion_feed_per_country_system_diet.csv")) %>%
  dplyr::select(SPAM_super = SPAM_super, GAEZ_category, diet, ingredient = source_ingredient, iso3c_producing, total_prop_ms = prop_produced_for_system_ms, total_prop_ge = prop_produced_for_system_ge) %>%
...
todo <- unique(dplyr::select(total_prop, diet, GAEZ_category, ingredient, allocation_type))


todo <- total_prop %>%
  group_by(diet, GAEZ_category, ingredient, allocation_type) %>%
...
feed_crop <- filter(total_prop, GAEZ_category==crop_gaez, diet == diet_type, ingredient == product_type , allocation_type == allocation) %>%
...
writeRaster(tonnes_crop_feed_raster, filename= sprintf(here("prep/02_feed/output/%s/%s_%s_%s_P.tif"), diet_type, crop_gaez, product_type, allocation), overwrite=TRUE)
...
writeRaster(area_crop_feed_raster, filename= sprintf(here("prep/02_feed/output/%s/%s_%s_%s_A.tif"), diet_type, crop_gaez, product_type, allocation), overwrite=TRUE)
...
test_names <- str_replace_all(str_before_last(list.files(here("prep/02_feed/output/plant-dominant/"), pattern = "mass_A.tif"), ".tif"), "_", ":")

test <- rast(list.files(here("prep/02_feed/output/plant-dominant/"), full.names = TRUE, pattern = "mass_A.tif"))

names(test) <- test_names
plot(log(test+1))
test_df <- as.data.frame(global(test, "sum", na.rm = TRUE))
sum(test_df$sum) # 10438.04 km2 of cropland for salmon aquaculture feeds? 
# 2929747 tonnes of crops for salmon aquaculture feeds 
# 2804335 demanded.. so pretty close there. 
2929747/10438.04 # 280.6798 tonnes/km2 
10438.04/2929747 # 0.003562779 km2/tonne 
(10438.04*1000000)/(2929747*1000) # 3.562779 m2 per kg.. EU chicken meat is 5.59 m2 per kg
...
feed_source <- read.csv(here("prep/02_feed/data/demand/ingredient_crop_demand_production_conversions.csv")) %>%
  ...
  group_by(iso3c_producing, diet, GAEZ_category, ingredient) %>%
