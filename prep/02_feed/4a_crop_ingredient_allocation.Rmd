...
todo <- total_prop %>%
  ...
  ungroup() %>%
  filter(sum_prop >0) 
todo <- filter(todo, fcr_type == "efficient")
...
test_names <- str_replace_all(str_before_last(list.files(here("prep/02_feed/output/plant-dominant/regular"), pattern = "mass_A.tif"), ".tif"), "_", ":")

test <- rast(list.files(here("prep/02_feed/output/plant-dominant/regular"), full.names = TRUE, pattern = "mass_A.tif"))
...
sum(test_df$sum) # 9240.518 km2 of cropland for salmon aquaculture feeds? 

# 3182323 demanded.
3182323/9240.518 # 344.3879 tonnes/km2 
9240.518/3182323 # 0.002903702 km2/tonne 
(9240.518*1000000)/(3182323*1000) # 2.903702 m2 per kg.. EU chicken meat is 5.59 m2 per kg
