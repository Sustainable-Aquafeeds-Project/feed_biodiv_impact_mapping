...
consumption <- read_csv(here("prep/02_feed/data/demand/total_aquaculture_ingredient_demand.csv")) %>%
  ...
  pivot_longer(cols = c("ingredient_demand", "ingredient_demand_efficient"), names_to = "demand_type", values_to = "consuming_ingredient_consumed_tonnes") %>%
  mutate(demand_type = ifelse(demand_type == "ingredient_demand", "regular", "efficient")) %>% 
  select(iso3c_consuming = iso3c, animal_system, diet, source_ingredient, demand_type, consuming_ingredient_consumed_tonnes) 
...
tonnes_extra_crop <- corrected_prop %>%
  dplyr::select(iso3c_producing, diet, SPAM_super, GAEZ_category, extra_crop_tonnes_ms, tonnes_producing_crop, extra_crop_tonnes_ge, extra_crop_tonnes_econ, source_ingredient) %>%
  unique() %>%
...
assigned_prop <- tonnes_included_system  %>%
  dplyr::select(iso3c_producing, diet, source_ingredient, SPAM_super, GAEZ_category, prop_produced_for_system_ms = included_prop_ms, prop_produced_for_system_ge = included_prop_ge, prop_produced_for_system_econ = included_prop_econ)
excess_prop <- excess_prop %>%
  dplyr::select(iso3c_producing, diet, source_ingredient, SPAM_super, GAEZ_category, prop_extra_for_system_ms = prop_dispersed_ms, prop_extra_for_system_ge = prop_dispersed_ge, prop_extra_for_system_econ = prop_dispersed_econ)
