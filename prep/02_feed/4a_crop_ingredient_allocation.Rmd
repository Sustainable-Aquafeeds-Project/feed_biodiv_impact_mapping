...
fao_yields <- readRDS(here("data/tidy_data/production-data/crops_production_tidy.rds")) %>%
  ...
  mutate(yield_tonnes_ha = value/10000) %>% # convert hg to kg 
...
area_crop_feed_raster <- tonnes_crop_feed_raster/yield_feed_crop_raster
...
# global(area_crop_feed_raster, "sum", na.rm = TRUE) # 342455.7 hectares of soybean for SPC??
...
test_names <- str_replace_all(str_before_last(list.files(here("prep/02_feed/output/plant-dominant/"), pattern = "_A.tif"), ".tif"), "_", ":")

test <- rast(list.files(here("prep/02_feed/output/plant-dominant/"), full.names = TRUE, pattern = "_A.tif"))
