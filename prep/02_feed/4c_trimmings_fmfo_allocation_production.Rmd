...
setdiff(trimmings_spp$scientific_name, unique(trimmings_catch_iso$TaxonName)) # [1] "Thunnini sp"         "Sardinella spp"      "Engraulis spp"       ".. many of the common names associated with these match thouhg.
...
trimmings_allocations_df <- trimmings_allocation_tidy %>%
  ...
  add_row(CommonName = "Chub mackerel", TaxonName = "Scomber japonicus", ingredient = "fish meal, cut offs", ge_value = 0.7644580, mass_value = 1.5021600) %>%
    add_row(CommonName = "Chub mackerel", TaxonName = "Scomber japonicus", ingredient = "fish oil, cut offs", ge_value = 0.7644580, mass_value = 1.5021600) %>%
...
sardine_mean_om_mass <- trimmings_allocations_df %>%
  filter(CommonName == "Atlantic sardine", ingredient == "fish oil, cut offs") %>%
  pull(mass_value)
...
trimmings_fofm_allocation <- trimmings_fofm_origins %>%
  ...
  mutate(ge_value = case_when(
    ...
    is.na(ge_value) & str_detect(CommonName, "anchovy|Anchovies|anchovies|") & ingredient == "fish oil, cut offs" ~ anchovy_mean_fo_ge,
      ...
      is.na(ge_value) & str_detect(CommonName, "Sard|sard") & ingredient == "fish meal, cut offs" ~ anchovy_mean_fm_ge,
    is.na(ge_value) & str_detect(CommonName, "Sard|sard") & ingredient == "fish oil, cut offs" ~ anchovy_mean_fo_ge,
  TRUE ~ ge_value
  ),
  mass_value = case_when(
   ...
   is.na(mass_value) & str_detect(CommonName, "anchovy|Anchovies|anchovies|") & ingredient == "fish oil, cut offs" ~ anchovy_mean_fo_mass, 
     is.na(mass_value) & str_detect(CommonName, "Sard|sard") & ingredient == "fish meal, cut offs" ~ anchovy_mean_fm_mass,
   is.na(mass_value) & str_detect(CommonName, "Sard|sard") & ingredient == "fish oil, cut offs" ~ anchovy_mean_fo_mass,
   TRUE ~ mass_value
  )) %>%
  mutate(ge_value = case_when(
...
sum(trimmings_fofm_allocation$tonnes_fofm_trimmings_spp_live_weight) # 2571538/2 = 1285769 Ok this makes sense. 

mean(trimmings_fofm_allocation$allocation_value) # 1.808569

1285769/652000 # 1.972038 perfect 
...
trim_files <- list.files(here("prep/02_feed/output/fish-dominant/"), pattern = "fish oil, cut offs|fish meal, cut offs", full.names = TRUE)
...
# 1 fish-dominant  ge              fish meal, cut offs 444717.
# 2 fish-dominant  ge              fish oil, cut offs  535928.
# 3 fish-dominant  mass            fish meal, cut offs 530246.
# 4 fish-dominant  mass            fish oil, cut offs  316429.
# 5 plant-dominant ge              fish meal, cut offs 181121.
# 6 plant-dominant ge              fish oil, cut offs  218269.
# 7 plant-dominant mass            fish meal, cut offs 215955.
# 8 plant-dominant mass            fish oil, cut offs  128873.
