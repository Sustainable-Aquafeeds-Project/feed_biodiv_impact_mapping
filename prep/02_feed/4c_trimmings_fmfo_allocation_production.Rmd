...
sum(country_demand$ingredient_demand) # 1105093/2


sum(forage_demand$ingredient_demand, na.rm = TRUE) # 3565490/2

1105093/3484838 # 0.3099415 trimmings demand represent ~30% of all FMFO in diet
...
sum(trimmings_fofm_origins$tonnes_fofm_trimmings_spp, na.rm = TRUE) # 1091781 losing ~13k bc of Uruguay is missing... they don't report catch for any of the trimmings spp we have... 
...
sum(trimmings_fofm_allocation$tonnes_fofm_trimmings_spp_live_weight) # 5071584/3 = 1690528 Ok this makes sense. 

mean(trimmings_fofm_allocation$allocation_value) # 1.429135

1690528/1091781 # 1.548413 seems fine 
...
# trimmingsfish_2 <- trimmingsfish %>% 
#   filter(trimmings_catch > 0) %>%
#   filter(iso3c %in% c(unique(trim_props$iso3c)))
...
for(diet_type in diets){
  ...
  for(allocation in allocations){
    ...
    #   fcr = "regular"
        ...
        ## comment this in if u want to skip those already run
...
trim_files <- trim_files[grep("_A", trim_files)]
