...
sum(country_demand$ingredient_demand) # 660114.3


sum(forage_demand$ingredient_demand, na.rm = TRUE) # 2129803

660114.3/2129803 # 0.3099415 trimmings demand represent ~30% of all FMFO in diet
...
sum(demand_trade_origin$total_ingredient_tonnes_traded, na.rm = TRUE) # 660114.3 good, the same as above. 
...
trimmings_fofm_origins <- trimmings_catch_iso %>%
  ...
  group_by(iso3c, ingredient, diet) %>%
...
sum(trimmings_fofm_origins$tonnes_fofm_trimmings_spp, na.rm = TRUE) # 651674.8 losing ~9k bc of Uruguay is missing... they don't report catch for any of the trimmings spp we have... 
...
trim_test_2 <- demand_trade_origin %>% 
  filter(partner_iso3c %in% c(setdiff(demand_trade_origin$partner_iso3c, trimmings_test$iso3c))) # yup this is because of these missing countries, without trimming spp
...
sum(trimmings_fofm_allocation$tonnes_fofm_trimmings_spp_live_weight) # 3058633/3 = 1019544 Ok this makes sense. 

mean(trimmings_fofm_allocation$allocation_value) # 1.429135

1019544/652000 # 1.563718 perfect 
...
for(diet_type in diets){
  for(allocation in allocations){
    ...
    #  pos = "benthic"
        ...
        if(!file.exists(sprintf(here("prep/02_feed/output/%s/trimmings fish_%s_%s_%s_P.tif"), diet_type, ingredient_type, allocation, pos))){
...
# print(sprintf("observed: %s", trimmings_fofm_allocation_totals %>% filter(diet == diet_type, allocation_type == allocation, ingredient == ingredient_type) %>% group_by(ingredient) %>% summarise(total = sum(total_tonnes_fofm_lw, na.rm = TRUE)) %>% ungroup() %>% pull(total)))
    ...
    writeRaster(trimmings_fish_system, sprintf(here("prep/02_feed/output/%s/trimmings fish_%s_%s_%s_P.tif"), diet_type, ingredient_type, allocation, pos), overwrite = TRUE)
        }else{
          cat("file exists")
        }
...
trim_files <- list.files(here("prep/02_feed/output/fish-dominant"), pattern = "fish oil|fish meal|trimmings", full.names = TRUE, recursive = TRUE)
trim_fish_orig <- trim_files[!grepl("benthic|pelagic|forage|_A", trim_files)]

trim_rasts <- rast(trim_fish_orig)

trim_names <- basename(trim_fish_orig)

names(trim_rasts) <- trim_names

plot(log(trim_rasts+1))

global(trim_rasts, "sum", na.rm = TRUE)
#                                                         sum
# trimmings fish_fish meal, cut offs_economic_P.tif  56660.98
# trimmings fish_fish meal, cut offs_ge_P.tif       549960.60
# trimmings fish_fish meal, cut offs_mass_P.tif     527769.88
# trimmings fish_fish oil, cut offs_economic_P.tif   43884.05
# trimmings fish_fish oil, cut offs_ge_P.tif        678742.13
# trimmings fish_fish oil, cut offs_mass_P.tif      316429.29



trim_fish_new <- trim_files[grepl("benthic|pelagic|trimmings|_P", trim_files)]
trim_fish_new <- trim_fish_new[!grepl("archive|forage|_A", trim_fish_new)]

new_rasts <- rast(trim_fish_new)

new_names <- basename(trim_fish_new)
...
#                                                                  sum
# trimmings fish_fish meal, cut offs_economic_benthic_P.tif    142.136
# trimmings fish_fish meal, cut offs_economic_pelagic_P.tif  56518.849
# trimmings fish_fish meal, cut offs_ge_benthic_P.tif         2355.600
# trimmings fish_fish meal, cut offs_ge_pelagic_P.tif       547604.998
# trimmings fish_fish meal, cut offs_mass_benthic_P.tif       2290.481
# trimmings fish_fish meal, cut offs_mass_pelagic_P.tif     525479.402
# trimmings fish_fish oil, cut offs_economic_benthic_P.tif    1095.001
# trimmings fish_fish oil, cut offs_economic_pelagic_P.tif   42789.050
# trimmings fish_fish oil, cut offs_ge_benthic_P.tif         28170.557
# trimmings fish_fish oil, cut offs_ge_pelagic_P.tif        650571.572
# trimmings fish_fish oil, cut offs_mass_benthic_P.tif       11436.686
# trimmings fish_fish oil, cut offs_mass_pelagic_P.tif      304992.609

142.136 + 56518.849 # 56660.99 - perfect! it matches
...
#    diet           allocation_type ingredient              sum
#    <chr>          <chr>           <chr>                 <dbl>
#  1 fish-dominant  economic        fish meal, cut offs  56661.
#  2 fish-dominant  economic        fish oil, cut offs   43884.
#  3 fish-dominant  ge              fish meal, cut offs 549961.
#  4 fish-dominant  ge              fish oil, cut offs  678742.
#  5 fish-dominant  mass            fish meal, cut offs 527770.
#  6 fish-dominant  mass            fish oil, cut offs  316429.
#  7 plant-dominant economic        fish meal, cut offs  23076.
#  8 plant-dominant economic        fish oil, cut offs   17873.
#  9 plant-dominant ge              fish meal, cut offs 223984.
# 10 plant-dominant ge              fish oil, cut offs  276433.
# 11 plant-dominant mass            fish meal, cut offs 214946.
# 12 plant-dominant mass            fish oil, cut offs  128873.
