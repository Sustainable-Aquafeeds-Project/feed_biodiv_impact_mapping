...
crop_production_gf <- crop_production_gf %>%
  dplyr::select(iso3c_producing, GAEZ_category, imports_per = prop_global_tonnes) 
...
feed_products <- read_csv(here("prep/01_crops_prep/data/GAEZ_names.csv")) %>%
  left_join(mapspam_feed_products, by = c("item_code" = "FAO_item_code")) %>% # match to GAEZ categories
  mutate(FAO_item_code_NFB = ifelse(GAEZ_category == "Cotton", 2559, FAO_item_code_NFB)) %>% 
...
discards <- read_csv(here("prep/02_feed/data/FAOSTAT_FBS_Items_discard_5-9-2022.csv")) %>% # made by hand
  filter(cut=="yes") %>%
  pull(`Item Code`)
...
filter(local_prod, consuming_country == "Afghanistan" & GAEZ_category == "Sorghum")
filter(local_prod, consuming_country == "Ethiopia" & GAEZ_category == "Barley")
...
Join GAEZ crop production data with fbs data.  If there is no local production according to GAEZ,
...
filter(local_prod_gf, iso3c_consuming == "AFG" & GAEZ_category == "Sorghum")
...
feed_products_full <- read_csv(here("prep/01_crops_prep/data/GAEZ_names.csv")) %>%
...
feed_products <- feed_products_full %>% # match to GAEZ categories
  ...
  unique() %>%
  add_row(GAEZ_category = "Cassava",
          FAO_Item_Code = 128) %>% # because cassava leaves are reported differently in the trade matrix, we will assume that all cassava is dried in the country that they are produced and exported from
  add_row(GAEZ_category = "Cotton", 
          FAO_Item_Code = 329) %>% # because raw seed cotton is not in the trade matrix, we will assume that all cotton seeds are extracted in the country that they are produced and exported from
  add_row(GAEZ_category = "Oilpalmfruit",
          FAO_Item_Code = 256) # because raw oil palm fruit is not in the trade matrix, we will assume that all palm kernels are extracted in the country that they are produced and exported from.. 
...
## ok might need to adjust cotton seed, cassava, and oilpalmfruit
...
filter(imports, iso3c_consuming=="MYS" & GAEZ_category=="Oilpalmfruit")
...
filter(imports_prop, iso3c_consuming == "AFG" & GAEZ_category == "Sunflower")
filter(imports_prop, iso3c_consuming == "ETH" & GAEZ_category == "Barley")
filter(imports_prop, iso3c_consuming == "FJI" & GAEZ_category == "Pulses")
...
sum(tmp$prop_of_supply)  # Mapspam was 0.004... GAEZ is 0.003834579... perfect
...
tmp <- filter(trade_data, iso3c_consuming == "USA", GAEZ_category == "Cotton") %>% arrange(-prop_of_supply)
...
data.frame(dplyr::filter(check, total!=1)) # that's fine, all 0.99...

write_csv(trade_data, here("prep/02_feed/data/FAO_GAEZcrop_trade_data.csv"))

test <- read.csv(here("prep/02_feed/data/archive/FAO_MAPSPAMcrop_trade_data.csv")) %>%
  filter(iso3c_consuming == "MYS")

test2 <- read.csv(here("prep/02_feed/data/FAO_GAEZcrop_trade_data.csv")) 

## most everything looks the same... 

test2 <- read.csv(here("prep/02_feed/output/norway_trade.csv")) 
