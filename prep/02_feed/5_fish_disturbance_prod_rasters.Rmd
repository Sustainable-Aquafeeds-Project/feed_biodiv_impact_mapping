...
# mean_npp <- rast(here("prep/02_feed/data/vgpm-npp/npp_2013_2022_mean.tif"))
# median_npp <- rast(here("prep/02_feed/data/vgpm-npp/npp_2013_2022_median.tif"))
# min_npp <- rast(here("prep/02_feed/data/vgpm-npp/npp_2013_2022_min.tif"))
# max_npp <- rast(here("prep/02_feed/data/vgpm-npp/npp_2013_2022_max.tif")) 

old_2019 <- rast("/mnt/rdsi/raw_data/00-net-primary-productivity/annual_mean_npp_2019_gf_wgs.tiff")
new_2019 <- rast(here("../npp_synthesis/data/npp_2019/mean_npp_2019.tif"))
...
#Separate the catch records that have Genus and species.
trim_spp <- readRDS(file.path(watson_dir, "v5.0/int/all_catch_trimmingsfish.rds")) |> 
...
trim_higherorder <- readRDS(file.path(watson_dir, "v5.0/int/all_catch_trimmingsfish.rds")) |> 
  filter(trimmingsfish == 1) |> 
  mutate(binomial = stringi::stri_count(TaxonName, regex="\\S+")) |> 
  filter(binomial==1) |>  
  pull(TaxonName) |> 
...
test <- readRDS(file.path(watson_dir, "v5.0/int/all_catch_trimmingsfish.rds")) %>% 
  filter(trimmingsfish == 1)
...
NOTES: These area numbers look very wrong compared to Rich's. I'm not sure what is happening. In terms of embodied tonnage, Rich's values are higher (I'm pretty sure), so I'm not sure why my area values would be so much higher...
...
forage_npp <- foragefish %>% 
  ...
  mutate(fmfo_embodied_fish = prop*FOFM_catch) %>% 
    ...
    left_join(cell_area) |> 
    mutate(mean_npp_t_C_yr = mean_npp_t_km2_yr*OceanAreasqkm) |> 
    mutate(median_npp_t_C_yr = median_npp_t_km2_yr*OceanAreasqkm) |> 
    mutate(min_npp_t_C_yr = min_npp_t_km2_yr*OceanAreasqkm) |> 
    mutate(max_npp_t_C_yr = max_npp_t_km2_yr*OceanAreasqkm) |>
    mutate(ppr_fmfo = (fmfo_embodied_fish/carbon_conversion)*(1/te)^(DietTroph-1)) %>% #T C yr-1
    as_tibble() |> 
    mutate(ppr_prop_fmfo = case_when(ppr_fmfo/max_npp_t_C_yr<1 ~ ppr_fmfo/max_npp_t_C_yr,
                                        TRUE ~ ppr_fmfo/max_npp_t_C_yr)) |> 
    arrange(LonCentre, LatCentre, diet, allocation_type, ingredient) |> 
    left_join(cell_area) |> 
    mutate(area_fmfo = ppr_prop_fmfo*OceanAreasqkm)
...
test <- forage_npp %>%
  filter(diet == "plant-dominant",
        ...
        # str_detect(TaxonName, "Euphausia superba")
         ) %>%
  dplyr::select(LonCentre, LatCentre, fmfo_embodied_fish) # %>%
  # dplyr::select(LonCentre, LatCentre, fmfo_embodied_fish)

test2 <- rast(test, type = "xyz")
 plot(test2)
plot(log(test2+1))

sum(test$fmfo_embodied_fish) # 1093060 tonnage
sum(test$area_fmfo, na.rm = TRUE) # 2563.028 ok that is way less than rich... need to see what trimmings adds since he includes trimmings too... 

## compare to cashion.. these numbers seem pretty large 

compare <- rast(here("prep/02_feed/output/plant-dominant/forage fish_fish meal_mass_P.tif"))
global(compare, "sum", na.rm = TRUE) # 1093060 tonnage matches up.
 

compare_rich <- rast(list.files(here("prep/02_feed/test/plant-dominant/"), full.names = TRUE, pattern = "km2"))

sum(global(compare_rich, "sum", na.rm = TRUE)) # 17446.36 km2 way more than me. But he has double the fish meal tonnage reported...


compare_rich_tonnes <- rast(list.files(here("prep/02_feed/test/plant-dominant/"), full.names = TRUE, pattern = "diet"))

sum(global(compare_rich_tonnes, "sum", na.rm = TRUE)) # 3948399 tonnes way more than me. > double the fish meal tonnage reported...

test_forage <- 
 forage_npp %>% 
  filter(allocation_type == "mass",
         diet == "plant-dominant",
         ingredient == "fish meal") %>%
  dplyr::select(CommonName, area_fmfo, fmfo_embodied_fish)
...
sum(test$fmfo_embodied_fish) # 380101.2 tonnage
sum(test$area_fmfo, na.rm = TRUE) # 32232.51 ok maybe this will be a bit more inline with Rich? 


#Bind the forage and trimmings catch npp per cell

all_fish_npp <- 
  bind_rows(forage_npp |> select(diet, allocation_type, ingredient, LonCentre, LatCentre, area_fmfo, fmfo_embodied_fish),
            trim_npp |> select(diet, allocation_type, ingredient, LonCentre, LatCentre, area_fmfo, fmfo_embodied_fish)) |> 
  group_by(diet, allocation_type, ingredient, LonCentre, LatCentre) |> 
...
sum(test$fmfo_embodied_fish) # 1473161 tonnage
sum(test$area_fmfo, na.rm = TRUE) # 34795.54 
...
sum(global(compare_rich, "sum", na.rm = TRUE)) # 17446.36 km2 ok I have about double what Rich has... Most of mine is coming from trimmings though which seems odd...


compare_rich_tonnes <- rast(list.files(here("prep/02_feed/test/plant-dominant/"), full.names = TRUE, pattern = "diet"))

sum(global(compare_rich_tonnes, "sum", na.rm = TRUE)) # 3948399 tonnes way more than me. > double the fish meal tonnage reported... so what is going on??

plot(log(app(compare_rich, "sum", na.rm = TRUE)+1))

combine_rich <- app(compare_rich, "sum", na.rm = TRUE)
