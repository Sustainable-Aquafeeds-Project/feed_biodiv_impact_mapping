...
forage_npp <- foragefish %>% 
  ...
  dplyr::select(LonCentre, LatCentre, diet, allocation_type, ingredient, water_col_position, fmfo_embodied_fish, area_fmfo, area_fmfo_bycatch)
...
all_fish_npp <- 
  bind_rows(forage_npp |> select(diet, allocation_type, ingredient, water_col_position, LonCentre, LatCentre, area_fmfo, fmfo_embodied_fish, area_fmfo_bycatch),
            trim_npp |> select(diet, allocation_type, ingredient, water_col_position, LonCentre, LatCentre, area_fmfo, fmfo_embodied_fish, area_fmfo_bycatch)) |> 
...
for(diet_type in diets){
  ...
  if(str_detect(ingredient_type, "cut offs")){
      ...
      writeRaster(rast_xyz, sprintf(here("prep/02_feed/output/%s/%s_%s_%s_%s_catch_A.tif"), diet_type, fish_type, ingredient_type, allocation, pos), overwrite = TRUE)
      
            rast_xyz_bycatch <- rast(rast_xyz_df_bycatch, type = "xyz", crs = crs(base_rast)) %>%
        project(base_rast)
      
      writeRaster(rast_xyz_bycatch, sprintf(here("prep/02_feed/output/%s/%s_%s_%s_%s_bycatch_A.tif"), diet_type, fish_type, ingredient_type, allocation, pos), overwrite = TRUE)
...
test <- rast(here("prep/02_feed/output/plant-dominant/archive/trimmings fish_fish meal, cut offs_mass_pelagic_A.tif"))
plot(log(test+1))

global(test, "sum", na.rm = TRUE) # 2464.607 km2


test_ben <- rast(here("prep/02_feed/output/plant-dominant/trimmings fish_fish meal, cut offs_mass_benthic_catch_A.tif"))
test_pel <- rast(here("prep/02_feed/output/plant-dominant/trimmings fish_fish meal, cut offs_mass_pelagic_catch_A.tif"))
