...
# FOFM_props <- FOFM_props %>% 
#   filter(fcr_type == "efficient") # just running efficient right now
...
## had to run trim_npp and forage_npp separately due to memory constraints...
# diets <- unique(trim_npp$diet)
# ingredients <- unique(trim_npp$ingredient)
# allocations <- unique(trim_npp$allocation_type)
# depth_positions <- unique(trim_npp$water_col_position)
# fcrs <- unique(trim_npp$fcr_type)
...
# 
# global(test_ben, "sum", na.rm = TRUE) + global(test_pel, "sum", na.rm = TRUE)  # 2513.923 km2

test_ben <- rast(here("prep/02_feed/output/plant-dominant/regular/trimmings fish_fish meal, cut offs_mass_benthic_catch_A.tif"))
test_pel <- rast(here("prep/02_feed/output/plant-dominant/regular/trimmings fish_fish meal, cut offs_mass_pelagic_catch_A.tif"))

global(test_ben, "sum", na.rm = TRUE) + global(test_pel, "sum", na.rm = TRUE)  # 2284.717 km2
