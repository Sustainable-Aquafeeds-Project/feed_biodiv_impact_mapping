...
# mean_npp <- rast(here("../npp_synthesis/data/npp_2019/mean_npp_2019.tif"))
# median_npp <- rast(here("../npp_synthesis/data/npp_2019/median_npp_2019.tif"))
# min_npp <- rast(here("../npp_synthesis/data/npp_2019/min_npp_2019.tif"))
# max_npp <- rast(here("../npp_synthesis/data/npp_2019/max_npp_2019.tif"))
...
forage_npp <- foragefish %>% 
  ...
  mutate(fmfo_embodied_fish = prop*FOFM_catch) %>%
    ...
    mutate(ppr_prop_fmfo = ppr_fmfo/max_npp_t_C_yr) |> 
    mutate(area_fmfo = ppr_prop_fmfo*area) %>%
  group_by(Cell, diet, allocation_type, ingredient) %>%
  summarise(area_fmfo = sum(area_fmfo, na.rm = TRUE),
            fmfo_embodied_fish = sum(fmfo_embodied_fish, na.rm = TRUE)) %>%
  ungroup() %>% 
  left_join(watson_cells) %>%
  dplyr::select(LonCentre, LatCentre, diet, allocation_type, ingredient, fmfo_embodied_fish, area_fmfo)
...
for(diet_type in diets){
  for(ingredient_type in ingredients){
    for(allocation in allocations){
      ...
      fish_type = ifelse(ingredient_type %in% c("fish oil" ," fish meal"), "forage fish", "trimmings fish")
      
      rast_xyz <- rast(rast_xyz_df, type = "xyz", crs = crs(base_rast)) %>%
        project(base_rast)
      
      writeRaster(rast_xyz, sprintf(here("prep/02_feed/output/%s/%s_%s_%s_A.tif"), diet_type, fish_type, ingredient_type, allocation), overwrite = TRUE)
...
test <- rast(here("prep/02_feed/output/plant-dominant/trimmings fish_fish meal, cut offs_mass_A.tif"))
plot(log(test+1))

global(test, "sum", na.rm = TRUE) # 28807.79 perfect! 
```  
...
testing <- rast(here("prep/02_feed/output/plant-dominant/forage fish_fish meal_mass_P.tif"))
plot(log(testing+1))

sum(test$fmfo_embodied_fish) # 1156102 tonnage
sum(test$area_fmfo, na.rm = TRUE) # 113203.9 ok that is more than rich... we are supposed to have more, but does that much more make sense? need to see what trimmings adds since he includes trimmings too... 
...
global(test2, "sum", na.rm = TRUE) # 28807.79 WHOOOOOOOOOOOO

sum(test$fmfo_embodied_fish) # 204494.7 tonnage
sum(test$area_fmfo, na.rm = TRUE) # 28807.79 

trim_catch <- rast(here("prep/02_feed/output/plant-dominant/trimmings fish_fish meal, cut offs_mass_P.tif"))
plot(trim_catch)
...
sum(test$fmfo_embodied_fish) # 1360597 tonnage
sum(test$area_fmfo, na.rm = TRUE) # 142011.7

1360597/142011.7 # 9.58088
...
sum(global(compare_rich, "sum", na.rm = TRUE)) # 17446.36 km2 ok lines up well with what Rich has... Most of mine is coming from trimmings though which seems odd...

plot(log(app(compare_rich, "sum", na.rm = TRUE)+1))
combine_rich <- app(compare_rich, "sum", na.rm = TRUE)

100000/17446.36 # = 5.731855


# so to make Rich's, my total should be ~1277342/5.731855 = 222849.7 km2. 
...
sum(kuempel_fofm_dist$value) # 80902.86... surprisingly similar lol
```
