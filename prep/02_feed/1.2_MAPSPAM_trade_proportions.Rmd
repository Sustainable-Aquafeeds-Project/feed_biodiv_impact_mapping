...
crop_production_df <- read_csv( here("prep/02_feed/data/MAPSPAMcrop_production.csv")) # made in the last script
...
crop_production_gf <- crop_production_gf %>%
 # filter(prop_global_tonnes > 0) %>%
  # filter(!(SPAM_super %in% c("toba", "xcof", "teas", "othr", "spis"))) %>% # lets filter out the things we don't need for aquaculture feeds
...
feed_products <- read_csv(here("prep/02_feed/data/MapSPAM_to_FAO_v2.csv")) %>%
  filter(!is.na(FAO_item_code_NFB)) %>%
    filter(is.na(non_food)) %>%
  dplyr::select(SPAM_super, FAO_Item_Code_NFB = FAO_item_code_NFB) %>%
  unique()
...
discards <- read_csv(here("prep/02_feed/data/FAOSTAT_FBS_Items_discard_5-9-2022.csv")) %>% # not sure how Mel made this... I think by hand.
  filter(cut=="yes") %>%
  pull(`Item Code`)
...
filter(local_prod_gf, iso3c_consuming == "ETH" & SPAM_super == "rape")
...
feed_products_full <- read_csv(here("prep/02_feed/data/MapSPAM_to_FAO_v2.csv"))
...
filter(imports, iso3c_consuming=="FJI" & SPAM_super=="sunf")
...
imports <- left_join(imports, crop_production_df, by=c("iso3c_producing", "SPAM_super")) %>%
  ...
  dplyr::select(iso3c_consuming, iso3c_producing, SPAM_super, imports_tonnes)

filter(imports, iso3c_consuming=="AFG" & SPAM_super=="opul")
```
...
filter(imports_prop, iso3c_consuming == "AFG" & SPAM_super == "sunf")
...
imports_new_data <- filter(local_prod_rgn_crop_list, major_group_prod %in% missing_trade) %>%
  dplyr::select(iso3c_consuming, SPAM_super)
...
sum(tmp$prop_of_supply)  # should be 0.0055 (that was 2017); now it is 0.004... close enough
...
write_csv(trade_data, here("prep/02_feed/data/FAO_MAPSPAMcrop_trade_data.csv"))

test <- read.csv(here("prep/02_feed/data/FAO_MAPSPAMcrop_trade_data.csv")) %>%
  filter(iso3c_consuming == "NOR")
