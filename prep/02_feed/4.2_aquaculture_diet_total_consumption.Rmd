...
Now calculate ingredient demand per diet (just modern diet for now) and for each salmon producing country. While we are not really interested in this project in the country-specific data, global demand is affected by the among country differences in FCR.
...
tonnes_consumption <- feed_demand %>%
  ...
  mutate(product = "meat") %>%
  dplyr::select(iso3c, animal= species, system = production_system, product, ingredient = raw_name, ingredient_demand) 
...
combine_feedstuff_corrected <- combine_demand %>%
 left_join(fofm) %>%
  ...
  dplyr::select(iso3c, animal, system, product, ingredient, ingredient_demand=tonnes_feedstuff_fofm_correct)
