...
fao <- read_csv(here("prep/01_crops_mapspam/data/FAOSTAT_crop_production_2010_2021.csv")) %>% 
  ...
  filter(area != "China",
         area_code < 5000) %>%
  dplyr::select(area, item_code, value) %>%
...
compare
sum(compare$our_tonnes, na.rm=TRUE)/sum(compare$total) # teas and rest categories are problematic... everything else looks ok though. 
