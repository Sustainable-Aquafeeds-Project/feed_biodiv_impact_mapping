...
tmp <- filter(production_location, iso3c_consuming=="CHL"  & animal_system == "salmon_aquaculture_meat") %>%
    group_by(SPAM_super) %>%
  slice_max(order_by = producing_crop_consumed_tonnes, n = 15) %>%
  ungroup()
#%>%
 # filter(producing_crop_consumed_tonnes > 1000)
tmp
summary(tmp)
...
ggplot(data = tmp, aes(x = producing_crop_consumed_tonnes, y = reorder_within(iso3c_producing, producing_crop_consumed_tonnes, SPAM_super))) + 
  geom_bar(stat = "identity") + 
  facet_wrap(~SPAM_super, scales = "free") + 
  labs(y = "Country") + 
  theme_bw()

test <- filter(production_location, iso3c_producing == "NOR" & animal_system == "salmon_aquaculture_meat")
...
global_exceed <- proportion %>%
  ...
  dplyr::select(SPAM_super, prop_consumed)
