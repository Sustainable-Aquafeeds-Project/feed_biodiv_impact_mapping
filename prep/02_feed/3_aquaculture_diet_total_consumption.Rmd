...
test <- read.csv(here("prep/02_feed/data/demand/total_aquaculture_ingredient_demand.csv")) %>%
  filter(ingredient %in% c("fish oil, forage fish", "fish meal, forage fish")) %>%
  group_by(diet) %>%
  summarise(total = sum(ingredient_demand, na.rm = TRUE)) %>% 
  ungroup()


test2 <- read.csv(here("prep/02_feed/data/demand/total_global_ingredient_demand.csv"))
```
