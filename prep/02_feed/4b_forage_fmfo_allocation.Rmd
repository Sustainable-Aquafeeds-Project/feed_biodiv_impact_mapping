...
baci_raw <- read.csv(file.path(rdsi_raw_data_dir, "BACI_data/BACI_HS12_V202301/BACI_HS12_Y2017_V202301.csv")) # want the 2017 data to match fisheries 
...
baci_filt <- baci_raw %>% 
  ...
  # filter(str_detect(description, "oil|meal|Oil|Meal")) %>%
  # filter(str_detect(description, "fish|Fish")) %>%
  rename(importer_code = j, partner_code = i, code = k) %>%
  filter(code %in% c(230120, 150420)) %>%
  left_join(country_codes, by = c("importer_code" = "country_code")) %>%
  dplyr::select(year = t, importer_code, importer_name = country_name_full, importer_iso3c = iso_3digit_alpha, partner_code, hs_code = code, v, q, description) %>%
...
length(unique(oils_meals_import_props$partner_iso3c)) # 124 not unreasonable to think. The FOFM demand is only like 10 countries. 

length(unique(oils_meals_import_props$importer_iso3c)) # 171
...
unique(demand_trade$partner_iso3c) # 85 countries
```
...
test <- rast(list.files(here("prep/02_feed/output/plant-dominant/"), full.names = TRUE, pattern = "fish oil|fish meal"))
test_names <- list.files(here("prep/02_feed/output/plant-dominant/"), pattern = "fish oil|fish meal")
