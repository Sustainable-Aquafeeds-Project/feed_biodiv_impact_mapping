...
unique(raw_230120_meals$ReporterISO) # 78 importing countries..
unique(raw_230120_meals$Partner2ISO) # 81 exporting? Maybe just not a lot of FM producers? 
...
length(unique(raw_150420_oils$ReporterISO)) # 76 countries.. 
length(unique(raw_150420_oils$PartnerCode)) # 162 countries.. thats better 


raw_150410_oils <- read.csv(here("prep/02_feed/data/comtrade/TradeData_150410.csv"))

length(unique(raw_150410_oils$ReporterISO)) # 75 countries.. 
length(unique(raw_150410_oils$PartnerCode)) # 141 countries.. 
...
oils_df <- raw_150420_oils %>% ## we'll use the fats and oils other than liver oil...
    filter(ReporterISO != "W00", 
         PartnerISO != "W00") %>%
...
length(unique(oils_meals_import_props$PartnerISO)) # 113. not unreasonable to think. The FOFM demand is only like 10 countries. 
...
forage_fofm_origins_meal <- forage_catch_iso %>%
  ...
  left_join(demand_trade_origin %>% filter(ingredient == "fish meal"), by = c("iso3c" = "PartnerISO")) %>%
...
forage_fofm_origins_oil <- forage_catch_iso_no_krill %>%
  dplyr::select(iso3c, CommonName, TaxonName, prop_of_iso_total) %>%
  left_join(demand_trade_origin %>% filter(ingredient == "fish oil"), by = c("iso3c" = "PartnerISO")) %>%
  mutate(tonnes_fofm_spp = total_ingredient_tonnes_traded*prop_of_iso_total) %>%
  dplyr::select(iso3c, CommonName, TaxonName, ingredient, diet, tonnes_fofm_spp) %>%
  filter(!is.na(tonnes_fofm_spp))
