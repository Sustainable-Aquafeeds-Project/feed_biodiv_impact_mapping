...
I think I need to add disturbance calculation somewhere in here. 
...
#   
#   foragefish_system <- left_join(foragefish, system_prop, by = c("iso3c", "TaxonName")) %>% ## think i need to stop and save this here so that I can calculate disturbance by spp and cell 
...
for(diet_type in diets){
  ...
  foragefish_system <- left_join(foragefish, system_prop, by = c("iso3c")) %>% ## think i need to stop and save this here so that I can calculate disturbance by spp and cell 
    mutate(prop = ifelse(is.na(prop), 0, prop)) %>%
    mutate(FOFM_catch_foodsystem = prop*FOFM_catch) %>%
    group_by(Cell) %>%
    summarise(tonnes = sum(FOFM_catch_foodsystem, na.rm = TRUE)) %>%
