...
sum(country_demand$ingredient_demand) # 2598876
...
sum(demand_trade$tonnes_ingredient_demand_traded, na.rm = TRUE) # 2515503... losing a little bit because missing FRO... FRO isn't in the BACI data :( consider gapfilling BACI data for FRO with ISL? 
```
...
sum(demand_trade_origin$total_ingredient_tonnes_traded) # 2515503
...
sum(country_demand$ingredient_demand) # 2598876... missing ~100k tonnes... 
...
sum(forage_fofm_allocation$tonnes_fofm_spp_live_weight) # 19066438/2 = 9533219.. seems excessive, ~4x more than the demand. Do fish weight 4x more than meal and oil? 

mean(forage_fofm_allocation$allocation_value) # 3.946963.. yeah, i guess they do. 
...
print(sprintf("observed: %s", forage_fofm_allocation_totals %>% filter(diet == diet_type, allocation_type == allocation, ingredient == ingredient_type) %>% group_by(ingredient) %>% summarise(total = sum(total_tonnes_fofm_lw, na.rm = TRUE)) %>% ungroup() %>% pull(total)))
    ...
    writeRaster(forage_fish_system, sprintf(here("prep/02_feed/output/%s/forage fish_%s_%s_P.tif"), diet_type, ingredient_type, allocation), overwrite = TRUE)
...
forage_files <- list.files(here("prep/02_feed/output/plant-dominant/"), pattern = "fish oil|fish meal", full.names = TRUE)[1:4]

test <- rast(forage_files)

test_names <- basename(forage_files)
...
#                                        sum
# forage fish_fish meal_ge_P.tif    929948.7
# forage fish_fish meal_mass_P.tif 1093059.8
# forage fish_fish oil_ge_P.tif    1552807.4
# forage fish_fish oil_mass_P.tif  1023593.8
...
# 5 plant-dominant ge              fish meal   929949.
# 6 plant-dominant ge              fish oil   1548199.
# 7 plant-dominant mass            fish meal  1093060.
# 8 plant-dominant mass            fish oil   1020567.
