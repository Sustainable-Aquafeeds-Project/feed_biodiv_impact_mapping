...
for(diet_type in diets){
  ...
  foragefish_system <- left_join(foragefish, system_prop, by = c("iso3c", "TaxonName")) %>%
  ...
  forage_fish_system <- rast(foragefish_system, type = "xyz", crs = crs(watson_raster_template)) %>%
...
# print(sprintf("raster calc: %s", global(forage_fish_system, "sum", na.rm=TRUE)))
# print(sprintf("observed: %s", forage_fofm_allocation_totals %>% filter(diet == diet_type, allocation_type == allocation, ingredient == ingredient_type) %>% group_by(ingredient) %>% summarise(total = sum(total_tonnes_fofm_lw, na.rm = TRUE)) %>% ungroup() %>% pull(total)))
    ...
    writeRaster(forage_fish_system, sprintf(here("prep/02_feed/output/%s/forage fish_%s_%s_%s_P.tif"), diet_type, ingredient_type, allocation, pos), overwrite = TRUE)
...
forage_files <- list.files(here("prep/02_feed/output/fish-dominant"), pattern = "fish oil|fish meal", full.names = TRUE)
forage_fish_orig <- forage_files[!grepl("benthic|pelagic|trimmings|_A", forage_files)]

orig_rasts <- rast(forage_fish_orig)

orig_names <- basename(forage_fish_orig)

names(orig_rasts) <- orig_names

plot(log(orig_rasts+1))

global(orig_rasts, "sum", na.rm = TRUE)
#                                          sum
# forage fish_fish meal_economic_P.tif 2665081
# forage fish_fish meal_ge_P.tif       2495606
# forage fish_fish meal_mass_P.tif     2838613
# forage fish_fish oil_economic_P.tif  3158491
# forage fish_fish oil_ge_P.tif        3776819
# forage fish_fish oil_mass_P.tif      2609488
...
125450.2+2539630.9 # 2665081 - perfect it adds up! 

test <- forage_fofm_allocation %>%
  group_by(diet,
         allocation_type,
         ingredient) %>%
  summarise(sum = sum(tonnes_fofm_spp_live_weight)) %>%
  ungroup()
test
