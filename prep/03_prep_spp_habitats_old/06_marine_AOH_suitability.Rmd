...
overlay_vuln <- overlay_df %>%
  ...
  mutate(bycatch_quartile = case_when(
   ...
   mutate(bycatch_0.5 = case_when(
    bycatch_gf <0.25 ~ 0,
    bycatch_gf <= 0.75 & bycatch >= 0.25 ~ 0.25, 
    bycatch_gf > 0.75 ~ 0.5
  )) %>%
  mutate(affected_reg_vuln = aoh_overlap*bycatch_quartile) %>%
  mutate(affected_rescale_vuln = aoh_overlap*bycatch_0.5) %>%
  mutate(affected_depth = ifelse(!(species %in% c(trimmings_spp$species, forage_spp$species)), aoh_overlap*vuln_depth, aoh_overlap)) %>%
  mutate(affected_depth_reg_vuln = ifelse(!(species %in% c(trimmings_spp$species, forage_spp$species)), aoh_overlap*vuln_depth*bycatch_quartile, aoh_overlap*bycatch_quartile)) %>%
  mutate(affected_depth_rescale_vuln = ifelse(!(species %in% c(trimmings_spp$species, forage_spp$species)), aoh_overlap*vuln_depth*bycatch_0.5, aoh_overlap*bycatch_0.5)) %>% 
  dplyr::select(Genus, Species, spp_type, diet, allocation, ingredient, spp_depth_pos, aoh_area_orig, reg_vuln = bycatch_quartile, vuln_rescale = bycatch_0.5, vuln_depth, aoh_overlap, affected_reg_vuln, affected_rescale_vuln, affected_depth, affected_depth_reg_vuln, affected_depth_rescale_vuln) %>%
