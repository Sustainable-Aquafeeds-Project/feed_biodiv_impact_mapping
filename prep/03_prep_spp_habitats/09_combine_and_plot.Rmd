...
muted <- unname(c(muted(9)))[c(5,6,7,8,9)]
...
maps <- map(.x = diets, 
  ...
  geom_tile(data = loop_df  %>% 
              filter(diet == x), aes(x= long, y = lat, fill = value)) +  # mutate(exp(value) - 1) / 1000000)
...
ingredients <- all_overlap %>% 
  filter(str_detect(ingredient, "fish")) %>% 
...
ggplot(summary_df, aes(x = raw_material, y = impact_total, fill = spp_type)) +
  ...
  labs(title = "AOH Affected (55236 species); Economic allocation", x = "Raw material", y = expression(paste("Total AOH Affected (", km^{2}, ")")), fill = NULL, caption = "A") +
...
summary_df$raw_material <- with(summary_df, reorder(raw_material, impact_w_mean, FUN = function(x) sum(x)))


ggplot(summary_df, aes(x = raw_material, y = impact_w_mean, fill = spp_type)) +
    geom_bar(stat = "identity") +
  facet_grid(fcr_type ~ diet, scales = "free_y") +
  labs(title = "Species weighted mean AOH Affected (55236 species); Economic allocation", x = "Raw material", y = "km2 per species impacted", fill = NULL, caption = "B") +
  # scale_fill_manual(values = category_colors) +
  scale_fill_manual(values = muted_palette) + 
  theme_minimal() + 
  coord_flip()  + 
...
summary_df$raw_material <- with(summary_df, reorder(raw_material, mean_ext_risk, FUN = function(x) sum(x)))


ggplot(summary_df, aes(x = raw_material, y = mean_ext_risk, fill = spp_type)) +
    geom_bar(stat = "identity") +
  facet_grid(fcr ~ diet, scales = "free_y") +
  labs(title = "Species weighted mean extinction risk (55236 species); Economic allocation", x = "Raw material", y = "Species weighted mean extinction risk", fill = NULL, caption = "B") +
  # scale_fill_manual(values = category_colors) +
  scale_fill_manual(values = muted_palette) + 
  theme_minimal() + 
  coord_flip()  + 
...
ggplot(summary_df, aes(x = ordered_ingredient, y = impact_total)) + 
  ...
  facet_grid(fcr_type ~ diet, scales = "free_y") +
    coord_flip() +
  labs(title = "Global AOH Affected (55236 species); Economic allocation", x = "Raw material and Ingredient", y = expression(paste("Total AOH Affected (", km^{2}, ")"))) + 
      theme(axis.text.x = element_text(angle = -90, hjust = 1)) # Adjust the 'angle' value as needed
...
ggplot(summary_df, aes(x = ordered_ingredient, y = impact_total, fill = spp_type)) +
    geom_bar(stat = "identity") +
  facet_grid(fcr_type ~ diet, scales = "free_y") +
  labs(title = "Global AOH Affected (55236 species); Economic allocation", x = "Raw material and Ingredient", y = expression(paste("Total AOH Affected (", km^{2}, ")")), fill = NULL) +
  # scale_fill_manual(values = category_colors) +
  scale_fill_manual(values = muted_palette) + 
  theme_minimal() + 
  coord_flip()  + 
...
ggplot(summary_df, aes(x = raw_material, y = impact_total, fill = spp_type)) +
    geom_bar(stat = "identity") +
  facet_grid(fcr_type ~ diet, scales = "free_y") +
  labs(title = "Global AOH Affected (55236 species); Economic allocation", x = "Raw material", y = expression(paste("Total AOH Affected (", km^{2}, ")")), fill = NULL) +
  # scale_fill_manual(values = category_colors) +
  scale_fill_manual(values = muted_palette) + 
  theme_minimal() + 
  coord_flip()  + 
