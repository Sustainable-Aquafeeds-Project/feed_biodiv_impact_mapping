...
# test <- data.frame(f = list.files(file.path(impact_maps_dir, "csvs/impact_maps_across_taxon_ingredient/"), full.names = TRUE)) %>%
...
if(str_detect(type, "mean")){
  ...
  delta_df <- delta_df %>%
    ...
    # group_by(long, lat, fcr_type, calc_type) %>%
    # summarise(`fish-dominant` = mean(`fish-dominant`, na.rm = TRUE),
    #           `plant-dominant` = mean(`plant-dominant`, na.rm = TRUE)) %>% 
    # ungroup() %>% 
...
## create maps in separate plots, force common scale between them
  ...
  theme_minimal() +
      theme(
          ...
          axis.text = element_blank(),
          axis.ticks = element_blank())
  ## make the delta a categorical variable for fish-dominant or plant-dominant? Regardless of magnitude? Set anything above the limit to be full red or blue? 
