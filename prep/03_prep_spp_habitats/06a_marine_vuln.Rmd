...
In this script we assign and gapfill vulnerability values for each species and stressor combination. Each species is vulnerable to either bycatch, biomass removal, or both, depending upon the ingredient which is being fished. 
...
aquamaps_spp_vuln_taxa <- aquamaps_spp_clean %>%
  ...
  mutate(taxon = case_when(
   ...
   is.na(taxon) & spp_type == "Reptiles and amphibians" ~ "Reptiles and amphibians",
  ...
  is.na(taxon) &  Class == "Cephalopoda" ~ "cephalopods",
   is.na(taxon) & Phylum == "Cnidaria" ~ "corals",
  ...
  mutate(taxon = case_when(
    ...
    taxon == "seabirds" ~ "Bird", 
    taxon == "reptiles" ~ "Reptiles and amphibians", 
    TRUE ~ taxon
  )) %>% 
...
Create final info dataset that can be used in the loop in the next script 
