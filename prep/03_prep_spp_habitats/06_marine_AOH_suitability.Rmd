...
overlay_vuln <- overlay_df %>%
  ...
  mutate(affected_depth = aoh_overlap*vuln_depth) %>%
  mutate(affected_depth_reg_vuln = aoh_overlap*vuln_depth*bycatch_quartile) %>%
  mutate(affected_depth_rescale_vuln = aoh_overlap*vuln_depth*bycatch_0.5) %>% 
...
sum(test$affected_depth, na.rm = TRUE) # 64466.13 
sum(test$affected_depth_reg_vuln, na.rm = TRUE) # 27780.4 
sum(test$affected_depth_rescale_vuln, na.rm = TRUE) # 13890.2 


mean(test$affected_reg_vuln, na.rm = TRUE) # 9.53995 km2 mean
mean(test$affected_rescale_vuln, na.rm = TRUE) # 4.769975 km2 mean 
mean(test$affected_depth, na.rm = TRUE) # 1.364334 km2 mean 
mean(test$affected_depth_reg_vuln, na.rm = TRUE) # 0.5879324 km2 mean 
mean(test$affected_depth_rescale_vuln, na.rm = TRUE) # 0.2939662 km2 mean 
