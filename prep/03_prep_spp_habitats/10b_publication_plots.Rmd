...
summary_info_s <- all_overlap %>%
  ...
  ungroup() %>% 
  mutate(total_hab = sum(spp_total_hab$total_hab_area)) %>%
  mutate(prop = impact_total/total_hab)
...
plot1 <- ggplot(summary_df_spp %>% filter(prop_impact > 0), aes(x = prop_impact)) +
  ...
  geom_point(data = summary_df_spp, aes(x = mean_prop, y = taxon, group = diet),
   ...
   geom_boxplot(aes(y = taxon, fill = diet), outlier.fill = NA, outlier.color = NA, position = position_dodge(width = -0.9), alpha = 0) +
  theme_ohara() +
...
ggsave(here("prep/03_prep_spp_habitats/output/plots/publication/economic_boxplot_prop.png"), width = 16, height = 9, units = "in") # technically for nature food the max width is 180 mm, however, I'm saving like this and adjusting save in post production because doing it here messing things up...
...
ggsave(here("prep/03_prep_spp_habitats/output/plots/publication/test.png"))
...
ggplot(summary_df_barcharts, aes(x = taxon, y = prop_impact, fill = raw_material)) +
    geom_bar(stat = "identity") +
  facet_wrap(~diet) +
  labs(x = "Taxon", y = "Proportion of habitat impacted", fill = NULL) +
  theme_minimal() + 
...
ggsave(here("prep/03_prep_spp_habitats/output/plots/publication/economic_barchart.png"), width = 178.9, height =100.92 , units = "mm")

## how efficient are these compared to their contribution to the feed formulation?
...
# ingredient_type = "Soybean_soybean meal"
        ...
        map_df <- rast(list.files(file.path(glue(impact_maps_dir, "/impact_maps_by_taxon_ingredient/{diet_type}/{fcr_type}")), pattern = glue("prop_unwt_{ingredient_type}_{allocation_type}_{spp_type}_mean.tif"), full.names = TRUE)) %>%
          ...
          group_by(rgn_id) %>%
          summarise(prop_mean = mean(prop_mean, na.rm = TRUE)) %>%
...
summary_df <- combined_impact_df %>%
  ...
  group_by(iso3c, diet, fcr) %>%
  mutate(total_impact = sum(prop_mean, na.rm = TRUE)) %>%
    left_join(top_25_impacts) %>%
    filter(!is.na(rank))
  
legend_plot <-  get_legend(ggplot(summary_df, aes(x = iso3c, y = prop_mean, fill = raw_material)) +
    geom_bar(stat = "identity") +
...
panel_1 <- ggplot(panel_1_df, aes(x = iso3c, y = prop_mean, fill = raw_material)) +
    geom_bar(stat = "identity") +
 scale_fill_manual(values = pal_mats) + 
  theme_minimal() + 
  coord_flip()  + 
...
panel_2 <- ggplot(panel_2_df, aes(x = iso3c, y = prop_mean, fill = raw_material)) +
    geom_bar(stat = "identity") +
 scale_fill_manual(values = pal_mats) + 
  theme_minimal() + 
  coord_flip()  + 
...
ggdraw() + 
  ...
  draw_label('Average proportion of habitat impacted', x = 0.35, y = .034, hjust = 0, vjust = 1, 
...
ggsave(here("prep/03_prep_spp_habitats/output/plots/publication/SI/economic_country_props.png"))
...
country_stats_1 <- combined_impact_df %>%
  ...
  summarise(sum_prop_mean = sum(prop_mean, na.rm = TRUE)) %>%
  ungroup() %>%
  group_by(diet, fcr) %>%
...
Lets look at average impacts and their proportional contributions, rather than at an individual species level

```{r}
## first need to pull all of the values 
## lets read in each mean proportion impacted raster as data frame and make mean plots per species and raw material for economic allocation
...
# ingredient_type = "Soybean_soybean meal"
        ...
        write.csv(map_df, file.path(glue(impact_maps_dir, "/csvs/impact_maps_by_taxon_ingredient_summary/mean_prop_impacts_{spp_type}_{allocation_type}_{diet_type}_{fcr_type}_{ingredient_type}.csv")), row.names = FALSE)
...
csv_files <- list.files(file.path(impact_maps_dir, "csvs/impact_maps_by_taxon_ingredient_summary"), pattern = "mean_prop", full.names = TRUE)
...
summary_df$raw_material <- with(summary_df, reorder(raw_material, prop_mean, FUN = function(x) mean(x)))


ggplot(summary_df, aes(x = raw_material, y = prop_mean, fill = taxon)) +
    geom_bar(stat = "identity") +
  facet_grid(fcr ~ diet, scales = "free_y") +
...
summary_df$taxon <- with(summary_df, reorder(taxon, prop_mean, FUN = function(x) sum(x)))

ggplot(summary_df, aes(x = taxon, y = prop_mean, fill = raw_material)) +
    geom_bar(stat = "identity") +
  facet_grid(fcr ~ diet, scales = "free_y") +
  labs(caption = "Economic allocation", x = "Taxon", y = "Average proportion of habitat imapcted", fill = NULL) +
  theme_minimal() + 
  scale_fill_manual(values = pal_mats) + 
  coord_flip()  + 
    theme(axis.text.x = element_text(angle = -90, hjust = 1)) +
...
ggsave(here("prep/03_prep_spp_habitats/output/plots/publication/SI/economic_average_props.png"))


## look at stats

summary_df_stats <- summary_df %>%
