...
In this script we summarize the impact rasters created in 03b, 06a and 06b by finding the mean, standard deviation (pooled variance), and number of species:
...
already_done <- data.frame(f = list.files(file.path(impact_maps_dir, "impact_maps_across_taxon_by_ingredient"), full.names = TRUE, recursive = TRUE)) %>%
  mutate(time = file.info(f)$ctime,
         size = file.size(f)) %>%
  mutate(type = ifelse(str_detect(f, "regular"), "regular", "efficient")) %>%
...
fcrs <- c("efficient") # add efficient for later
...
allocations <- c("mass", "ge", "economic")
diets <- c("fish-dominant", "plant-dominant")
fcrs <- c("efficient") # add efficient later
