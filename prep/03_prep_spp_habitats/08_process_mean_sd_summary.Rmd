...
taxa <- tx_impact_map_df %>% filter(t != "polychaetes") %>% 
  # filter(t %in% c("Terrestrial mammal", "Bird", "Reptiles", "Amphibians", "cnidaria", "corals")) %>% 
  pull(t) %>% unique()
fcrs <- c("efficient", "regular")
...
allocations <- c("mass", "ge", "economic")
diets <- c("fish-dominant", "plant-dominant")
taxa <- tx_impact_map_df %>% filter(t != "polychaetes") %>% filter(t %in% c("Terrestrial mammal", "Bird", "Reptiles", "Amphibians", "cnidaria", "corals")) %>% pull(t) %>% unique()
fcrs <- c("efficient", "regular") 
