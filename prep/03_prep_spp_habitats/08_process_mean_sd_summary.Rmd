...
for(diet in diet_types){
  for(fcr in fcr_types){
    for(allocation in allocation_types){
      ...
      all_files_list <- lapply(all_files, readRDS)
 ...
 combined_dataframe <- bind_rows(i_dfs) %>%
  ...
  as.data.frame()
      ...
      writeRaster(rast_impact, glue(file.path(biodiv_dir, "int/terrestrial_aggregation/impact_maps_across_taxon_ingredient/{diet}/{fcr}/{allocation}_mean_prop.tif")))
        ...
        filter(!is.na(cell_id)) %>%
            ...
            writeRaster(rast_impact_sd, glue(file.path(biodiv_dir, "int/terrestrial_aggregation/impact_maps_across_taxon_ingredient/{diet}/{fcr}/{allocation}_sd_prop.tif")))
        ...
        filter(!is.na(cell_id)) %>%
            ...
            rast(., type = "xyz", crs = moll_template)
                  ...
                  writeRaster(rast_impact_nspp, glue(file.path(biodiv_dir, "int/terrestrial_aggregation/impact_maps_across_taxon_ingredient/{diet}/{fcr}/{allocation}_nspp.tif")))
...
second_dfs <- lapply(all_files_list, function(x) x[[2]])

# Step 2: Combine these dataframes into one
combined_dataframe <- bind_rows(second_dfs) %>%
    filter(!is.na(cell_id)) %>%
  as.data.table() %>%
  group_by(cell_id, species_full) %>%
  summarise(impact_km2 = sum(impact_km2, na.rm = TRUE)) %>% # divide by 100 after this for prop and cap prop at 1
...
for(diet in diet_types){
  for(fcr in fcr_types){
    for(allocation in allocation_types){
      ...
      for(file in all_files){
        ...
        # file = 1
               all_files_list <- lapply(all_files[[file]], readRDS)
               
               cell_chunk_df <- data.frame(cell_id = NA, species_full = NA, impact_km2 = NA)
  ...
  combined_dataframe <- bind_rows(i_dfs) %>%
    filter(!is.na(cell_id)) %>%
    dplyr::select(cell_id, species_full, impact_km2) %>%
    as.data.frame()
          
  cell_chunk_df <- rbind(cell_chunk_df, combined_dataframe)
