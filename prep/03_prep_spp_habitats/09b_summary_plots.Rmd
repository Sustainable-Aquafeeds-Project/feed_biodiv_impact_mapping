...
#   left_join(spp_assessed_plot_df)
# 
# summary_df_all_test <- all_overlap %>%
#   group_by(diet, fcr_type, allocation, taxon) %>%
#   summarise(
...
ggplot(summary_df, aes(x = taxon, y = mean_ext_risk, fill = raw_material)) +
    geom_bar(stat = "identity") +
  facet_grid(fcr ~ diet, scales = "free_y") +
  labs(title = "Species weighted mean extinction risk (55,236 species)", caption = "Economic allocation", x = "Raw material", y = "Species weighted mean extinction risk", fill = NULL) +
  theme_minimal() + 
...
p <- ggplot(summary_df, aes(x = taxon, y = mean_ext_risk, fill = raw_material)) +
    geom_bar(stat = "identity") +
  facet_grid(fcr ~ diet, scales = "free_y") +
  labs(x = "Raw material", y = "Species weighted mean extinction risk", fill = NULL) +
  theme_minimal() + 
  scale_fill_manual(values = pal) + 
  coord_flip()  + 
    theme(axis.text.x = element_text(angle = -90, hjust = 1))  + 
...
all_summary_df <- combined_impact_df %>%
    separate_wider_delim(ingredient_raw_material, delim = "_", names = c("raw_material", "ingredient")) %>%
  group_by(diet, fcr, allocation, spp_type) %>%
  summarise(
            mean_ext_risk = mean(mean_ext_risk, na.rm = TRUE)) %>%
  mutate(prop_risk = mean_ext_risk/sum(mean_ext_risk)) %>%
  ungroup() %>% 
...
ggplot(all_summary_df, aes(x = reorder(raw_material, match(raw_material, custom_order)), y = prop_risk, fill = spp_type)) +
    geom_bar(stat = "identity") +
  facet_grid(fcr ~ diet, scales = "free_y") +
...
custom_order <- c(sort(unique(all_summary_df$spp_type), decreasing = TRUE))


ggplot(all_summary_df, aes(x = reorder(spp_type, match(spp_type, custom_order)), y = prop_risk, fill = raw_material)) +
    geom_bar(stat = "identity") +
  facet_grid(fcr ~ diet, scales = "free_y") +
  labs(title = "Contribution of each harvest pressure to mean impact across species habitats by taxon", caption = "Economic allocation", x = "Raw material", y = "Proportion of total impact", fill = NULL) +
  #scale_fill_manual(values = color_scale) + 
