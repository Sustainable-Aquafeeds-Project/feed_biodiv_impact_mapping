...
**Extract extinction risk metrics from rasters**
...
#   left_join(spp_assessed_plot_df)
# 
summary_df_test <- all_overlap_df %>%
  group_by(diet, fcr, allocation, taxon, sciname) %>%
  summarise(
...
summary_df_all_test <- all_overlap_df %>%
  group_by(diet, fcr, allocation, taxon) %>%
  summarise(
            mean_ext_risk = mean(extinction_mean, na.rm = TRUE),
            med_ext_risk = median(extinction_mean, na.rm = TRUE)) %>%
...
test_quant <- quantile(summary_df_test$mean_ext_risk, 0.95)
test_quant <- quantile(summary_df_test$mean_ext_risk)

# summary_df_test$taxon <- with(summary_df_test, reorder(taxon, mean_ext_risk, FUN = function(x) median(x)))
...
summary_df_test$taxon <- factor(summary_df_test$taxon, levels = desired_order)

summary_df$taxon <- with(summary_df, reorder(taxon, mean_ext_risk, FUN = function(x) median(x)))
...
plot1 <- ggplot(summary_df_test, aes(x = mean_ext_risk)) +
  geom_boxplot(aes(y = taxon, fill = diet, color = diet), 
               width = 0.3, position = position_dodge(width = -0.7),
               size = 0.3, outlier.shape = NA) + #outlier.size = 0.3, outlier.color = NA) +
  geom_vline(xintercept = 0, color = "black") + 
  geom_point(data = summary_df_all_test, aes(x = mean_ext_risk, y = taxon, group = diet), 
             position = position_dodge(width = -0.7),
             shape = 21, color = "black", fill = "white", size = 1.5) +
    geom_point(data = summary_df_all_test, aes(x = med_ext_risk, y = taxon, group = diet), 
             position = position_dodge(width = -0.7),
             shape = "|", color = "black", fill = "white", size = 1.5) +
    facet_grid(~fcr, scales = "free_y", space = "free_y") + 
  theme_ohara() + 
      labs(x = "Mean Extinction Risk", fill = 'Diet', color = 'Diet') + 
           ## , title = "Distribution of mean extinction risk across species AOH") +   
  scale_x_continuous(expand = c(0,0), limits = c(0, test_quant)) +# , limits = c(0, max(summary_df$mean_ext_risk))) + 
  ...
  theme(panel.background = element_blank(),
      ...
      geom_text(data = spp_assessed_plot_df %>% mutate(fcr = "Regular"), aes(label = spp_count_str, y = taxon),
                x = .00003, hjust = 0, color = 'black', fill = NA,
                family = 'Helvetica',
                size = 2.2)



# summary_df$taxon <- with(summary_df, reorder(taxon, mean_ext_risk, FUN = function(x) mean(x)))
# 
# diet_cols = c("Fish-dominant" = "#0D6EB5", "Plant-dominant" = "#7BD67F")
# # diet_cols = c("fish-dominant" = "#0D6EB5", "plant-dominant" = "#7BD67F")
# 
...
# ## make boxplot of mean extinction risk
# 
# plot1 <- ggplot(summary_df, aes(x = mean_ext_risk)) +
#   geom_boxplot(aes(y = taxon, fill = diet, color = diet), 
#                width = 0.3, position = position_dodge(width = -0.7),
#                size = 0.3, outlier.size = 0.3, outlier.color = NA) +
#   geom_vline(xintercept = 0, color = "black") + 
#   geom_point(data = summary_df_all, aes(x = mean_ext_risk, y = taxon, group = diet), 
#              position = position_dodge(width = -0.7),
#              shape = 21, color = "black", fill = "white", size = 1.5) +
#   facet_grid(~fcr, scales = "free_y", space = "free_y") + 
#   theme_ohara() + 
#       labs(x = "Mean Extinction Risk", fill = 'Diet', color = 'Diet') + 
#            ## , title = "Distribution of mean extinction risk across species AOH") +   
#   scale_x_continuous(expand = c(0,0), limits = c(0, .000035)) +# , limits = c(0, max(summary_df$mean_ext_risk))) + 
#   scale_fill_manual(values = diet_cols, breaks = names(diet_cols), guide = guide_legend(reverse = FALSE)) + 
#   scale_color_manual(values = diet_cols, breaks = names(diet_cols), guide = guide_legend(reverse = FALSE)) + 
#   theme(panel.background = element_blank(),
#             legend.text = element_text(size = 10),
...
#             axis.text.x = element_text(size = 6, angle = -90, hjust = 1),
#         legend.position = "none") + 
# # annotate('rect', data = summary_df_all %>% filter(fcr == "Regular"),
# #                xmin = .00003, xmax = .000035, ymin = 0.5, ymax = 13.5,
...
# plot1 <- tag_facet(p = plot1, tag_pool = LETTERS, open = "", close = "") + 
#   theme(strip.text = element_text())


# get legend

plot1_legend <- get_legend(ggplot(summary_df_test, aes(x = mean_ext_risk)) +
  geom_boxplot(aes(y = taxon, fill = diet, color = diet), 
               width = 0.3, position = position_dodge(width = -0.7),
...
ggdraw() + 
  ...
  draw_plot(plot1_legend,  x = .35, y = 0.25, height = .10, width = .15) +
   draw_label('A', x = 0.14, y = .965, hjust = 0, vjust = 1, 
             size = 9, fontfamily = 'Helvetica', fontface = 'bold') +
  draw_label('B', x = 0.575, y = .964, hjust = 0, vjust = 1, 
             size = 9, fontfamily = 'Helvetica', fontface = 'bold')

ggsave(here("prep/03_prep_spp_habitats/output/plots/publication/exclude_outliers_boxplot.png"), width = 16, height = 9, units = "in")
...
p <- ggplot(summary_df, aes(x = taxon, y = mean_ext_risk, fill = raw_material)) +
    geom_bar(stat = "identity") +
  facet_grid(fcr ~ diet, scales = "free_y") +
