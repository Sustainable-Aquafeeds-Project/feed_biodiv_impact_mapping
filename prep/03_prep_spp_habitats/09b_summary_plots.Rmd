...
plot1 <- ggplot(summary_df, aes(x = mean_ext_risk)) +
  ...
  theme_ohara() + 
      labs(x = "Mean Extinction Risk", fill = 'Diet', color = 'Diet', title = "Distribution of mean extinction risk across species AOH") +   
...
plot1_legend <- get_legend(ggplot(summary_df, aes(x = mean_ext_risk)) +
  geom_boxplot(aes(y = spp_type, fill = diet, color = diet), 
               width = 0.3, position = position_dodge(width = -0.7),
               size = 0.3, outlier.size = 0.3) + 
  theme_ohara() + 
    theme(panel.background = element_blank(),
...
summary_df$spp_type <- with(summary_df, reorder(spp_type, mean_ext_risk, FUN = function(x) sum(x)))

ggplot(summary_df, aes(x = spp_type, y = mean_ext_risk, fill = raw_material)) +
    geom_bar(stat = "identity") +
  facet_grid(fcr ~ diet, scales = "free_y") +
  labs(title = "Species weighted mean extinction risk (55236 species)", caption = "Economic allocation", x = "Raw material", y = "Species weighted mean extinction risk", fill = NULL) +
  # scale_fill_manual(values = color_scale) + 
  theme_minimal() + 
  coord_flip()  + 
    theme(axis.text.x = element_text(angle = -90, hjust = 1)) # Adjust the 'angle' value as needed
