...
overlay_df <- purrr::map_df(list.files(here("prep/03_prep_spp_habitats/int/aoh_overlay_chunks_terrestrial/"), full.names = TRUE), readRDS) %>% 
  ...
  separate_wider_delim(sciname, delim = "_", names = c("genus", "species", "bird_type"), too_few = "align_start") %>%
  unite(sciname, c(genus, species))

length(unique(overlay_df$sciname)) # 16133 - perfect! 
...
setdiff(overlay_df$sciname, impact_df$sciname)
...
impact_df_tidy <- impact_df %>%
    group_by(sciname, diet, allocation, crop_ingredient) %>%
