...
First, save habitat ares for each species. Additionally, find the habitat areas for any species that are missing (3617 of them, because they have no overlap)
...
length(unique(impact_df$sciname)) # 27911 - shouldn't there be more... ? 
...
## ok so i guess they all only have really small habitat areas... make sense
...
test_ag <- ag_suitability %>% 
  filter(species %in% c(missing_spp)) # ok... majority of them should theoretically be impacted... does this just mean there was literally no overlap at all? Yes, or they are not vulnerable.
