...
legend_pattern <-  get_legend(ggplot() + geom_col_pattern(plot_df_new,
             ...
             pattern_size = 0.5, pattern_spacing = 0.025, pattern_key_scale_factor = 0.5, pattern_color = "white") +
               theme(legend.position = "bottom",
                     legend.spacing.x = unit(0.28, 'cm')) + 
                 labs(pattern = "Diet") + 
                guides(pattern = guide_legend(title.position = "bottom", title.hjust = 0.5, ncol = 1)) +
               scale_pattern_manual(values=c('fish-dominant' = 'none', 'plant-dominant' = 'stripe'), 
                                    labels = c("Fish-dominant", "Plant-dominant")))


p <- ggdraw() + draw_plot(iucn_plot) + draw_plot(legend_iucn, x = 0.1, y = 0.005, width = 0.5, height = 0.1) + draw_plot(legend_pattern, x = 0.6, y = 0.005, width = 0.5, height = 0.1)

ggsave(here("prep/03_prep_spp_habitats/output/plots/publication/iucn_plot.png"), units = "in", width = 8, height = 8)
...
#         strip.text = element_text(size = 10), 
#         legend.position  = "right") +
#   scale_fill_manual(values = iucn_cat_pal) # ok great this works, now just need to figure out how to add pattern to one of the diet bars
