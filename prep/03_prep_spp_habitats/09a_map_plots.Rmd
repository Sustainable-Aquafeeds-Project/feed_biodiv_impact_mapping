...
Additionally, make a delta map, and just color either fish dominant, plant dominant, or no change as the legend. Have the legend colors be the same as the green and blue we use for the boxplot.
...
all_df_quant$category <- factor(all_df_quant$category, levels = c("0-24", "25-49", "50-74", "75-94", "95-99.9", "≥99.9"))
  ...
  quantile_plot <-  ggplot() + 
    ...
    theme_minimal() +
      theme(plot.title = element_text(hjust = 0.5), 
          ...
          axis.ticks = element_blank(),
          legend.margin = margin(r = 10),
          legend.position = "bottom") + # add a margin to legend text so it isn't cut off when saving 
  labs(fill = "Impact Quantile") + 
    ...
    ggsave(glue(here(this_dir, "output/plots/publication/economic_mean_prop_quantiles.png")), plot = quantile_plot, width = 8, height = 8, bg = "white")
...
delta_df_quant <- delta_df %>%
  left_join(quantiles_delta, by = "delta_cat") %>%
    mutate(category = case_when(
    abs(delta) < q25 ~ "0-24",
    abs(delta) >= q25 & abs(delta) < q50 ~ "25-49",
    abs(delta) >= q50 & abs(delta) < q75 ~ "50-74",
    abs(delta) >= q75 & abs(delta) < q95 ~ "75-94",
    abs(delta) >= q95 & abs(delta) < q999 ~ "95-99.9",
    abs(delta) >= q999 ~ "≥99.9"
  ))
...
delta_pal <- c(
  ...
  "Plant-dominant" = "#7BD67F"
   ...
   delta_plot <-  ggplot() + 
      geom_tile(data = delta_df_quant %>% filter(delta_cat == "Plant-dominant"), aes(x = long, y = lat, fill = category, color = category)) +
            geom_sf(data = countries_shp, fill = NA, colour = "grey")  +
        # scale_fill_manual(values = delta_pal) +
    # scale_colour_manual(values = delta_pal)
    scale_x_continuous(expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
    theme_minimal() +
      theme(plot.title = element_text(hjust = 0.5), 
          ...
          legend.margin = margin(r = 10),
          legend.position = "bottom") + # add a margin to legend text so it isn't cut off when saving 
  labs(fill = "Greater Impact") + 
     guides(color = "none",
             fill = guide_legend(title.position = "bottom", title.hjust = 0.5, nrow = 1)) +
     new_scale_color() +
     new_scale_fill() + 
           geom_tile(data = delta_df_quant %>% filter(delta_cat == "Fish-dominant"), aes(x = long, y = lat, fill = category, color = category))
   
         ggsave(glue(here(this_dir, "output/plots/publication/economic_mean_prop_quantiles_delta.png")),
                plot = plot_grid(quantile_plot, delta_plot, nrow = 2, labels = c("A", "B")), 
...
```{r}
  hist_plot_fin <- ggplot_gtable(ggplot_build(hist_plot))

  strip_both <- which(grepl('strip-', hist_plot_fin$layout$name))

k <- 1
for (i in strip_both) {
j <- which(grepl('rect', hist_plot_fin$grobs[[i]]$grobs[[1]]$childrenOrder))
hist_plot_fin$grobs[[i]]$grobs[[1]]$children[[j]]$gp$fill <- quantile_pal[k]
k <- k+1
}

grid.draw(hist_plot_fin)
```
