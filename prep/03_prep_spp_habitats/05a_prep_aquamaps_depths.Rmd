...
In this script we find depth information for all AquaMaps species. 
 ...
 - This is necessary for the next script, in which we clip downscaled and re-projected Aqua<aps species suitable habitat maps (>0.6 probability) (from 0.5 to ~10km resolution) to a bathymetric layer, so as to constrain neritic and shallow-water species to areas no deeper than 200 m (adapted from O'Hara et al. 2021). Save each as a csv with cell id and presence/absence column
