...
# Ok these are 3 problem species

# [1] "Abrocoma_boliviensis" "Abrothrix_olivaceus"  "Abrothrix_sanborni"  

aoh_files <- list.files(file.path(rdsi_raw_data_dir, "AOH_lumbierres"), full.names = TRUE, pattern = "Abrocoma_boliviensis")

this_spp_raster_proj <- rast(aoh_files) %>%
      project(moll_template, method = "near")  %>% 
...
if(nrow(this_spp_raster_proj) == 0){

this_spp_raster_ag <- rast(aoh_files) %>%
      aggregate(fact = 10, fun = "mean", na.rm = TRUE) %>% 
      project(moll_template, method = "near")  %>% 
      as.data.frame(xy=TRUE) %>%
      rename(prescence = 3) %>% 
