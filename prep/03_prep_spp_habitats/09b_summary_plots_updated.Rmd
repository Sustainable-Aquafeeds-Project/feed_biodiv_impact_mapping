...
test <- all_overlap %>%
  filter(spp_type %in% c("fish", "sponges", "arthropods", "cephalopods", "cnidaria", "molluscs", "echinoderms", "Marine plant", "corals", "elasmobranchs", "Marine mammal")) %>%
  distinct(sciname, total_hab_area)
test2 <- data.frame(duplicated(test$sciname)) # ok.. so none of the marine ones have duplicates for sciname or total_hab_area


test <- all_overlap %>%
  filter(spp_type %in% c("Amphibians", "Terrestrial mammal", "Reptiles", "Bird")) %>%
  distinct(sciname, total_hab_area)
test2 <- data.frame(duplicates = duplicated(test$sciname)) %>% filter(duplicates == TRUE) # ok.. yikes.. there are a lot of duplicates for terrestrial species... what is going wrong with the code? Maybe I just need to calculate total hab area in a separate script for each species? 
