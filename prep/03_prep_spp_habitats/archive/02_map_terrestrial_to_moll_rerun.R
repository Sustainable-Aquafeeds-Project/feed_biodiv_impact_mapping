...
# Get file information, including file sizes
file_info <- file.info(x)

# Extract file sizes in bytes
file_sizes <- file_info$size

# Combine file names and sizes into a data frame
result_df <- data.frame(File = x, Size_Bytes = file_sizes) %>%
  filter(Size_Bytes <= 300)
