...
trim_spp_targeted <- am_info %>%
  filter(species %in% c(trimmings_spp$species)) %>%
  pull(SpeciesID) %>%
  unique()

forage_spp_targeted <- am_info %>%
  filter(species %in% c(forage_spp$species)) %>%
  pull(SpeciesID) %>%
  unique()

spp_names_not_targeted <- setdiff(unique(am_info$SpeciesID), c(forage_spp_targeted, trim_spp_targeted))
...
# if both forage and trimmings species, then it is affected by catch
# all other species only affected by bycatch for both
# make a lookup table for each species ID 

lookup_table_spp_rast <- am_info %>%
  dplyr::select(SpeciesID, species) %>%
  mutate(trim_spp = ifelse(SpeciesID %in% c(trim_spp_targeted), 1, NA),
         for_spp = ifelse(SpeciesID %in% forage_spp_targeted, 1, NA)) %>%
  mutate(fish_oil_trim_rast = case_when(
    trim_spp == 1 & for_spp == 1 ~ "trimmings fish_fish oil_.*_catch_A",
    for_spp == 1 & is.na(trim_spp)  ~ "trimmings fish_fish oil_.*_bycatch_A",
    trim_spp == 1 & is.na(for_spp)  ~ "trimmings fish_fish oil_.*_catch_A",
    TRUE ~ "trimmings fish_fish oil_.*_bycatch_A"
  )) %>%
  mutate(fish_meal_trim_rast = case_when(
    trim_spp == 1 & for_spp == 1 ~ "trimmings fish_fish meal_.*_catch_A",
    for_spp == 1 & is.na(trim_spp)  ~ "trimmings fish_fish meal_.*_bycatch_A",
    trim_spp == 1 & is.na(for_spp)  ~ "trimmings fish_fish meal_.*_catch_A",
    TRUE ~ "trimmings fish_fish meal_.*_bycatch_A"
  )) %>% 
  mutate(fish_oil_forage_rast = case_when(
    trim_spp == 1 & for_spp == 1 ~ "forage fish_fish oil_.*_catch_A",
    for_spp == 1 & is.na(trim_spp)  ~ "forage fish_fish oil_.*_catch_A",
    trim_spp == 1 & is.na(for_spp)  ~ "forage fish_fish oil_.*_bycatch_A",
    TRUE ~ "forage fish_fish oil_.*_bycatch_A"
  )) %>%
  mutate(fish_meal_forage_rast = case_when(
    trim_spp == 1 & for_spp == 1 ~ "forage fish_fish meal_.*_catch_A",
    for_spp == 1 & is.na(trim_spp)  ~ "forage fish_fish meal_.*_catch_A",
    trim_spp == 1 & is.na(for_spp)  ~ "forage fish_fish meal_.*_bycatch_A",
    TRUE ~ "forage fish_fish meal_.*_bycatch_A"
  ))
...
for(i in seq(1, length(spp_names), chunk_size)) {
  ...
  print(paste("chunk ", i, " out of 23699 finished"))
