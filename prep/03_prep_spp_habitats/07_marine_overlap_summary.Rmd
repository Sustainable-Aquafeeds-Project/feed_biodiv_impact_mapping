...
all_catch_bycatch <- forage_catch_bycatch %>%
  dplyr::select(water_col_position, forage_prop_bycatch = prop_discards) %>%
  left_join(trimmings_catch_bycatch %>% dplyr::select(water_col_position, trimmings_prop_bycatch = prop_discards))
...
vuln_scores <- read.csv(here("prep/03_prep_spp_habitats/data/spp_vuln_butt/vuln_gapfilled_score.csv")) %>%
  dplyr::select(vuln_gf_id, bycatch, biomass_removal)
...
aquamaps_vuln_scores <- aquamaps_spp_vuln_taxa %>%
 ...
 mutate(across(class:genus, tolower)) %>%
  ...
  dplyr::select(species, class, order, family, genus, vuln_gf_id, SpeciesID, SpecCode, FBname, spp_type, bycatch_gf, biomass_removal_gf)
...
overlay_df <- purrr::map_df(list.files(here("prep/03_prep_spp_habitats/int/aoh_overlay_chunks_marine_old"), full.names = TRUE, recursive = TRUE), readRDS) %>%
  rename(SpeciesID = sciname) %>%
  left_join(aquamaps_spp)

length(unique(overlay_df$SpeciesID)) # 23694
...
23694*3*2*2*2*2 == nrow(overlay_df) # TRUE perfect! So we have all of the combinations we need. (Just need to fix those 5 species at some point...)
...
overlay_vuln <- overlay_df %>%
  ...
  left_join(all_catch_bycatch, by = c("dist_depth_pos" = "water_col_position")) %>%
    mutate(vuln_depth = ifelse(str_detect(ingredient, "trimmings"), trimmings_prop_bycatch, forage_prop_bycatch)) %>%
  ...
  mutate(vuln = case_when(
    ...
    trim_spp == 1 & is.na(for_spp) & str_detect(ingredient, "forage fish") ~ bycatch_gf,
    trim_spp == 1 & str_detect(ingredient, "trimmings fish") ~ biomass_removal_gf, 
    for_spp == 1 & is.na(trim_spp) & str_detect(ingredient, "trimmings fish") ~ bycatch_gf,
    for_spp == 1 & str_detect(ingredient, "forage fish") ~ biomass_removal_gf,
    TRUE ~ bycatch_gf
  )) %>%
  mutate(vuln_quartile = case_when(
   ...
   mutate(vuln_0.5 = case_when(
    vuln <0.25 ~ 0,
    vuln <= 0.75 & vuln >= 0.25 ~ 0.25, 
    vuln > 0.75 ~ 0.5
  )) %>%
  mutate(affected_reg_vuln = aoh_overlap*vuln_quartile) %>%
  mutate(affected_rescale_vuln = aoh_overlap*vuln_0.5) %>%
  mutate(affected_depth = ifelse(!(species %in% c(trimmings_spp$species, forage_spp$species)), aoh_overlap*vuln_depth, aoh_overlap)) %>%
  mutate(affected_depth_reg_vuln = ifelse(!(species %in% c(trimmings_spp$species, forage_spp$species)), aoh_overlap*vuln_depth*vuln_quartile, aoh_overlap*vuln_quartile)) %>%
  mutate(affected_depth_rescale_vuln = ifelse(!(species %in% c(trimmings_spp$species, forage_spp$species)), aoh_overlap*vuln_depth*vuln_0.5, aoh_overlap*vuln_0.5)) %>% 
  dplyr::select(Genus, Species, spp_type, diet, allocation, ingredient, spp_depth_pos, aoh_area_orig, reg_vuln = vuln_quartile, vuln_rescale = vuln_0.5, vuln_depth, aoh_overlap, affected_reg_vuln, affected_rescale_vuln, affected_depth, affected_depth_reg_vuln, affected_depth_rescale_vuln) %>%
...
test <- readRDS(here("prep/03_prep_spp_habitats/output/aoh_marine_overlap.rds"))
...
sum(test_terr$aoh_affected, na.rm = TRUE) # 1000315
mean(test_terr$aoh_affected, na.rm = TRUE) # 3.171222
...
sum(test$aoh_overlap, na.rm = TRUE) # 934851.3

# 3342859 km2... that's a lot compared to crops! But there are more species, with larger AOH ranges, and more dispersed harvesting area

sum(test$affected_reg_vuln, na.rm = TRUE) # 450772.2
sum(test$affected_rescale_vuln, na.rm = TRUE) # 225386.1 
sum(test$affected_depth, na.rm = TRUE) # 100037.3 
sum(test$affected_depth_reg_vuln, na.rm = TRUE) # 45095 
sum(test$affected_depth_rescale_vuln, na.rm = TRUE) # 22547.5 


mean(test$affected_reg_vuln, na.rm = TRUE) # 9.53995 km2 mean
mean(test$affected_rescale_vuln, na.rm = TRUE) # 4.769975 km2 mean 
mean(test$affected_depth, na.rm = TRUE) # 2.117147 km2 mean 
mean(test$affected_depth_reg_vuln, na.rm = TRUE) # 0.9543714 km2 mean 
mean(test$affected_depth_rescale_vuln, na.rm = TRUE) # 0.4771857 km2 mean 
