...
impact_vuln <- impact_df %>%
  ...
  dplyr::select(sciname= species, diet, fcr_type, allocation, ingredient, impact_total, vulnerability = vuln_quartile, spp_type = taxon, impact_total, total_hab_area)
...
sum(test_marine$impact_total, na.rm = TRUE) # 138520.3
mean(test_marine$impact_total, na.rm = TRUE) # 1.50376

```


```{r}
fcr_comparison_df <- impact_vuln %>% 
...
test <- fcr_comparison_df %>% 
  filter(percent_difference < 0) # "arthropods"    "molluscs"      "echinoderms"   "elasmobranchs"
```
