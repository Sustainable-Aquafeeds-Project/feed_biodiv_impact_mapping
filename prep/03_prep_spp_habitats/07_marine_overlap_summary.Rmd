...
length(unique(impact_df$SpeciesID)) # 20254 missing some spp.. however, we filtered out the species that aren't vulnerable, so this makes sense
...
hab_areas <- impact_vuln %>% ungroup() %>% distinct(sciname, total_hab_area) %>%
  group_by(sciname) %>%
  filter(total_hab_area == max(total_hab_area)) %>%
  ungroup()

impact_vuln <- impact_vuln %>%
...
sum(test_terr$impact_total, na.rm = TRUE) # 400940.2
mean(test_terr$impact_total, na.rm = TRUE) # 1.857908
...
sum(test_marine$impact_total, na.rm = TRUE) # 133105
mean(test_marine$impact_total, na.rm = TRUE) # 2.070416
