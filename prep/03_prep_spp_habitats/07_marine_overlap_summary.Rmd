...
length(unique(impact_df$SpeciesID)) # 23637 missing 62 spp
...
test <- aquamaps_spp %>% filter(SpeciesID %in% spp_to_run) # ok they are all bacteria and protozoa, which we don't include... 


impact_vuln <- impact_df %>%
  dplyr::group_by(species, diet, allocation, ingredient) %>%
  summarise(impact_total = sum(impact_total, na.rm = TRUE)) %>%
  ungroup() %>% 
  left_join(vuln_taxa) %>%
  dplyr::select(sciname= species, diet, allocation, ingredient, impact_total, vulnerability = vuln_quartile, spp_type = taxon, impact_total)
