...
author: "Rich Cottrell and Gage Clawson"
date: "02/03/2023"
...
test <- readRDS(file.path(sprintf(paste0(iucn_habitat_dir, "/spp_habitat_%s-%s.rds"), first_spp_index, last_spp_index)))
...
habs_100 <- all_habitats %>%
  filter(spp_id %in% spp_100$taxonid) %>%
  mutate(spp_id = as.numeric(spp_id)) %>%
  distinct()

threats_100 <- all_threats %>%
  filter(id %in% c(spp_100$taxonid)) %>%
  mutate(id = as.numeric(id)) %>%
  dplyr::select(-spp_id) %>%
  distinct()

habs_threats_spp<- habs_100 %>%
  left_join(threats_100, by = c("spp_id"= "id")) %>%
  left_join(spp_100, by = c("spp_id" = "taxonid")) %>%
  dplyr::select(-c(code, suitability, season, majorimportance, result.code, result.scope, result.severity, result.invasive, api_error, taxonomic_authority, population)) %>%
  distinct()
